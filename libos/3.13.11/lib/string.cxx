...
static void *check_bytes8( u8 *start, u8 value, unsigned int bytes)
{
 while (bytes) {
  if (*start != value)
   return (void *)start;
...
void *memchr_inv( void *start, int c, size_t bytes)
{
 u8 value = c;
 u64 value64;
 ...
 if (bytes <= 16)
                ...
                return check_bytes8((u8 *)start, value, bytes);
 ...
 while (words) {
  if (*(u64 *)start != value64)
                ...
                return check_bytes8((u8 *)start, value, 8);
 ...
 return check_bytes8((u8 *)start, value, bytes % 8);
