...
#else
                return STRUCT_INIT(ktime_t, tv64, KTIME_MAX);
#endif // NOTC
#endif
#ifdef NOTC
	return (ktime_t) { .tv64 = (s64)secs * NSEC_PER_SEC + (s64)nsecs };
#else
        return STRUCT_INIT(ktime_t, tv64, (s64)secs * NSEC_PER_SEC + (s64)nsecs);
#endif // NOTC
}
...
#else
#define ktime_sub(lhs, rhs) \
                ({ STRUCT_INIT(ktime_t, tv64, (lhs).tv64 - (rhs).tv64); })
#endif // NOTC
...
#else
#define ktime_add_ns(kt, nsval) \
                ({ STRUCT_INIT(ktime_t, tv64, (kt).tv64 + (nsval)); })
#endif // NOTC
...
#else
        ktime_t kt;
        kt.tv.sec = (s32)ts.tv_sec;
        kt.tv.nsec = (s32)ts.tv_nsec;
        return kt;
#endif // NOTC
}
...
static inline ktime_t timeval_to_ktime(const struct timeval tv)
{
	...
	return (ktime_t) { .tv = { .sec = (s32)tv.tv_sec,
        ...
        ktime_t kt;
        kt.tv.sec = (s32)ts.tv_sec;
        kt.tv.nsec = (s32)ts.tv_usec * NSEC_PER_USEC;
        return kt;
#endif // NOTC
}
...
#else
        timespec ts;
        ts.tv.sec = (time_t) kt.tv.sec;
        ts.tv.nsec = (long) kt.tv.nsec;
        return ts;
#endif // NOTC
}
