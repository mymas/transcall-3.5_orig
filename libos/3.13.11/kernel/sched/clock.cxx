...
{
 return (unsigned long long)(jiffies - ((unsigned long)(unsigned int) (-300*250)))
     * (1000000000L / 250);
}
;

__attribute__((__section__(".data..read_mostly"))) int sched_clock_running;


__attribute__((__section__(".data..read_mostly"))) int sched_clock_stable;




struct _sched_clock_data {
...
static __attribute__((section(".data..percpu" "..shared_aligned"))) __typeof__(struct _sched_clock_data) sched_clock_data __attribute__((__aligned__((1 << (6)))));





static inline struct _sched_clock_data *this_scd(void)

{
 return &(*({ unsigned long tcp_ptr__; do { void *__vpp_verify = (typeof((&(sched_clock_data)) + 0))0; (void)__vpp_verify; } while (0); asm ("add " "%%""gs"":" "%P" "1" ", %0" : "=r" (tcp_ptr__) : "m" (this_cpu_off), "0" (&(sched_clock_data))); (typeof(*(&(sched_clock_data))) *)tcp_ptr__; }));
}




static inline struct _sched_clock_data *cpu_sdc(int cpu)

{
 return &(*({ do { void *__vpp_verify = (typeof(((&(sched_clock_data))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(&(sched_clock_data))) *)(&(sched_clock_data)))); (typeof((typeof(*(&(sched_clock_data))) *)(&(sched_clock_data)))) (__ptr + (((__per_cpu_offset[cpu])))); }); }));
}

void sched_clock_init(void)
{
 u64 ktime_now = ((ktime_get()).tv64);
 int cpu;
 ...
 for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_possible_mask)), ((cpu)) < nr_cpu_ids;) {
  ...
  struct _sched_clock_data *scd = cpu_sdc(cpu);
...
static u64 sched_clock_local(struct _sched_clock_data *scd)
...
again:
 ...
 max_clock = wrap_max(old_clock, scd->tick_gtod + ((1000000000L +250/2)/250));

 clock = wrap_max(clock, min_clock);
 clock = wrap_min(clock, max_clock);

 if (({ (0); ({ __typeof__(*(((&scd->clock)))) __ret; __typeof__(*(((&scd->clock)))) __old = (((old_clock))); __typeof__(*(((&scd->clock)))) __new = (((clock))); switch ((sizeof(*((&scd->clock))))) { case 1: { u8 *__ptr = ( u8 *)(((&scd->clock))); asm (".pushsection .smp_locks,\"a\"\n" ".balign 4\n" ".long 671f - .\n" ".popsection\n" "671:" "\n\tlock; " "cmpxchgb %2,%1" : "=a" (__ret), "+m" (*__ptr) : "q" (__new), "0" (__old) : "memory"); break; } case 2: { u16 *__ptr = ( u16 *)(((&scd->clock))); asm (".pushsection .smp_locks,\"a\"\n" ".balign 4\n" ".long 671f - .\n" ".popsection\n" "671:" "\n\tlock; " "cmpxchgw %2,%1" : "=a" (__ret), "+m" (*__ptr) : "r" (__new), "0" (__old) : "memory"); break; } case 4: { u32 *__ptr = ( u32 *)(((&scd->clock))); asm (".pushsection .smp_locks,\"a\"\n" ".balign 4\n" ".long 671f - .\n" ".popsection\n" "671:" "\n\tlock; " "cmpxchgl %2,%1" : "=a" (__ret), "+m" (*__ptr) : "r" (__new), "0" (__old) : "memory"); break; } case 8: { u64 *__ptr = ( u64 *)(((&scd->clock))); asm (".pushsection .smp_locks,\"a\"\n" ".balign 4\n" ".long 671f - .\n" ".popsection\n" "671:" "\n\tlock; " "cmpxchgq %2,%1" : "=a" (__ret), "+m" (*__ptr) : "r" (__new), "0" (__old) : "memory"); break; } default: __cmpxchg_wrong_size(); } __ret; }); }) != old_clock)
  goto again;
...
static u64 sched_clock_remote(struct _sched_clock_data *scd)

{



 struct _sched_clock_data *my_scd = this_scd();
...
again:
 ...
 if (__builtin_expect(!!((s64)(remote_clock - this_clock) < 0), 1)) {
  ptr = &scd->clock;
...
u64 sched_clock_cpu(int cpu)
{
 ...
 if (__builtin_expect(!!(!sched_clock_running), 0))
  return 0ull;
