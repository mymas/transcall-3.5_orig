...
{
 printf("irq %d, desc: %p, depth: %d, count: %d, unhandled: %d\n",
  irq, desc, desc->depth, desc->irq_count, desc->irqs_unhandled);
 printf("->handle_irq():  %p, ", desc->handle_irq);
 print_symbol("%s\n", (unsigned long)desc->handle_irq);
 printf("->irq_data.chip(): %p, ", desc->irq_data.chip);
 print_symbol("%s\n", (unsigned long)desc->irq_data.chip);
 printf("->action(): %p\n", desc->action);
 if (desc->action) {
  printf("->action->handler(): %p, ", desc->action->handler);
  print_symbol("%s\n", (unsigned long)desc->action->handler);
 }

 if (desc->status_use_accessors & IRQ_LEVEL) printf("%14s set\n", "IRQ_LEVEL");
 if (desc->status_use_accessors & IRQ_PER_CPU) printf("%14s set\n", "IRQ_PER_CPU");
 if (desc->status_use_accessors & IRQ_NOPROBE) printf("%14s set\n", "IRQ_NOPROBE");
 if (desc->status_use_accessors & IRQ_NOREQUEST) printf("%14s set\n", "IRQ_NOREQUEST");
 if (desc->status_use_accessors & IRQ_NOTHREAD) printf("%14s set\n", "IRQ_NOTHREAD");
 if (desc->status_use_accessors & IRQ_NOAUTOEN) printf("%14s set\n", "IRQ_NOAUTOEN");

 if (desc->core_internal_state__do_not_mess_with_it & IRQS_AUTODETECT) printf("%14s set\n", "IRQS_AUTODETECT");
 if (desc->core_internal_state__do_not_mess_with_it & IRQS_REPLAY) printf("%14s set\n", "IRQS_REPLAY");
 if (desc->core_internal_state__do_not_mess_with_it & IRQS_WAITING) printf("%14s set\n", "IRQS_WAITING");
 if (desc->core_internal_state__do_not_mess_with_it & IRQS_PENDING) printf("%14s set\n", "IRQS_PENDING");
...
enum {
 ...
 _IRQF_MODIFY_MASK = (IRQ_TYPE_SENSE_MASK | IRQ_NOPROBE | IRQ_NOREQUEST | IRQ_NOAUTOEN | IRQ_MOVE_PCNTXT | IRQ_LEVEL | IRQ_NO_BALANCING | IRQ_PER_CPU | IRQ_NESTED_THREAD | IRQ_NOTHREAD | IRQ_PER_CPU_DEVID | IRQ_IS_POLLED),
};
static inline void
...
{
 if (__builtin_expect(!!(desc->irq_data.chip->irq_bus_lock), 0))
  desc->irq_data.chip->irq_bus_lock(&desc->irq_data);
}

static inline void chip_bus_sync_unlock(struct irq_desc *desc)
{
 if (__builtin_expect(!!(desc->irq_data.chip->irq_bus_sync_unlock), 0))
...
{
 return __irq_get_desc_lock(irq, flags, _true, check);
}

static inline void
irq_put_desc_busunlock(struct irq_desc *desc, unsigned long flags)
{
 __irq_put_desc_unlock(desc, flags, _true);
}

static inline struct irq_desc *
irq_get_desc_lock(unsigned int irq, unsigned long *flags, unsigned int check)
{
 return __irq_get_desc_lock(irq, flags, _false, check);
}

static inline void
irq_put_desc_unlock(struct irq_desc *desc, unsigned long flags)
{
 __irq_put_desc_unlock(desc, flags, _false);
}
...
extern struct radix_tree_root irq_desc_tree;
...
unsigned int kstat_irqs(unsigned int irq)
{
 ...
 if (!desc || !desc->kstat_irqs)
  return 0;
        ...
        ks_irqs = desc->kstat_irqs;

 for (((cpu)) = -1; ((cpu)) = cpumask_next(((cpu)), (cpu_possible_mask)), ((cpu)) < nr_cpu_ids;)
                sum += *(typeof(({ do { void *__vpp_verify = (typeof((((ks_irqs))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((ks_irqs))) *)((ks_irqs)))); (typeof((typeof(*((ks_irqs))) *)((ks_irqs)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); })))g_map(({ do { void *__vpp_verify = (typeof((((ks_irqs))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((ks_irqs))) *)((ks_irqs)))); (typeof((typeof(*((ks_irqs))) *)((ks_irqs)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); }), sizeof(*(({ do { void *__vpp_verify = (typeof((((ks_irqs))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((ks_irqs))) *)((ks_irqs)))); (typeof((typeof(*((ks_irqs))) *)((ks_irqs)))) (__ptr + (((__per_cpu_offset[(cpu)])))); }); }))));
