...
struct key_type {
 ...
 int (*vet_description)( char *description);
 ...
 int (*match)( struct key *key, void *desc);
 ...
 void (*describe)( struct key *key, struct seq_file *p);
 long (*read)( struct key *key, char *buffer, size_t buflen);
...
static inline int key_negate_and_link(struct key *key,
          ...
          struct key *instkey)
{
 return key_reject_and_link(key, timeout, 126, keyring, instkey);
}
...
extern int user_match( struct key *key, void *criterion);
extern void user_revoke(struct key *key);
extern void user_destroy(struct key *key);
extern void user_describe( struct key *user, struct seq_file *m);
extern long user_read( struct key *key,
...
kuid_t make_kuid(struct user_namespace *ns, uid_t uid)
{
 ...
 return (kuid_t){ map_id_down(&ns->uid_map, uid) };
}
;
...
kgid_t make_kgid(struct user_namespace *ns, gid_t gid)
{
 ...
 return (kgid_t){ map_id_down(&ns->gid_map, gid) };
}
;
...
kprojid_t make_kprojid(struct user_namespace *ns, projid_t projid)
{
 ...
 return (kprojid_t){ map_id_down(&ns->projid_map, projid) };
}
;
