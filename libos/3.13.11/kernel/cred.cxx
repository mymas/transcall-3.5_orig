...
static inline int read_cred_subscribers( struct cred *cred)
{
...
{
 no_printk("[%-5.5s%5u] ""__put_cred(%p{%d,%d})""\n", get_current()->comm, get_current()->pid , cred, atomic_read(&cred->usage), read_cred_subscribers(cred))
 ...
 (cred == get_current()->cred);
 (cred == get_current()->real_cred);
