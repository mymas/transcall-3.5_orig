...
{
 return bdi->capabilities & 0x00000200;
}
...
static inline struct gendisk *part_to_disk(struct hd_struct *part)
{
 if (__builtin_expect(!!(part), 1)) {
  if (part->partno)
   return ({ typeof( ((struct gendisk *)0)->part0.__dev ) *__mptr = (((&((part)->__dev))->parent)); (struct gendisk *)( (char *)__mptr - ((size_t) &((struct gendisk *)0)->part0.__dev) );});
  else
   return ({ typeof( ((struct gendisk *)0)->part0.__dev ) *__mptr = (((&((part)->__dev)))); (struct gendisk *)( (char *)__mptr - ((size_t) &((struct gendisk *)0)->part0.__dev) );});
 }
 return 0;
}

static inline void part_pack_uuid( u8 *uuid_str, u8 *to)
{
 int i;
 for (i = 0; i < 16; ++i) {
...
static inline int blk_part_pack_uuid( u8 *uuid_str, u8 *to)
{
...
static inline void part_stat_set_all(struct hd_struct *part, int value)
{
 ...
 for (((i)) = -1; ((i)) = cpumask_next(((i)), (cpu_possible_mask)), ((i)) < nr_cpu_ids;)
  memset(({ do { void *__vpp_verify = (typeof((((part->dkstats))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*((part->dkstats))) *)((part->dkstats)))); (typeof((typeof(*((part->dkstats))) *)((part->dkstats)))) (__ptr + (((__per_cpu_offset[(i)])))); }); }), value,
...
extern dev_t blk_lookup_devt( char *name, int partno);
...
{
 (atomic_long_read(&ioc->refcount) <= 0);
 (atomic_read(&ioc->active_ref) <= 0);
...
{
 return bio_alloc_bioset(gfp_mask, nr_iovecs, 0);
}

static inline struct bio *bio_clone_kmalloc(struct bio *bio, gfp_t gfp_mask)
{
 return bio_clone_bioset(bio, gfp_mask, 0);
...
static inline bool bio_is_rw(struct bio *bio)
{
 ...
 if (bio->bi_rw & (1ULL << __REQ_WRITE_SAME))
  return _false;
...
{
 if (bio->bi_rw & ((1ULL << __REQ_NOMERGE) | (1ULL << __REQ_STARTED) | (1ULL << __REQ_SOFTBARRIER) | (1ULL << __REQ_FLUSH) | (1ULL << __REQ_FUA)))
  return _false;
...
static inline unsigned bio_list_size( struct bio_list *bl)
{
 unsigned sz = 0;
 struct bio *bio;
...
extern void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) load_default_elevator_module(void);
...
extern int elevator_change(struct request_queue *, char *);
...
{
 return !(rw_flags & (1ULL << __REQ_WRITE)) || (rw_flags & (1ULL << __REQ_SYNC));
}
...
static inline bool rq_mergeable(struct request *rq)
{
 ...
 if (rq->cmd_flags & ((1ULL << __REQ_NOMERGE) | (1ULL << __REQ_STARTED) | (1ULL << __REQ_SOFTBARRIER) | (1ULL << __REQ_FLUSH) | (1ULL << __REQ_FUA)))
  return _false;
...
static inline bool blk_check_merge_flags(unsigned int flags1,
      unsigned int flags2)
{
 if ((flags1 & (1ULL << __REQ_DISCARD)) != (flags2 & (1ULL << __REQ_DISCARD)))
  return _false;

 if ((flags1 & (1ULL << __REQ_SECURE)) != (flags2 & (1ULL << __REQ_SECURE)))
  return _false;

 if ((flags1 & (1ULL << __REQ_WRITE_SAME)) != (flags2 & (1ULL << __REQ_WRITE_SAME)))
  return _false;
...
extern unsigned int blk_rq_err_bytes( struct request *rq);
...
{
 if (__builtin_expect(!!(cmd_flags & (1ULL << __REQ_DISCARD)), 0))
  return ({ typeof(q->limits.max_discard_sectors) _min1 = (q->limits.max_discard_sectors); typeof((~0U) >> 9) _min2 = ((~0U) >> 9); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; });

 if (__builtin_expect(!!(cmd_flags & (1ULL << __REQ_WRITE_SAME)), 0))
...
static inline void blk_flush_plug(struct task_struct *tsk)
{
 ...
 if (plug)
  blk_flush_plug_list(plug, _false);
}
...
static inline void blk_schedule_flush_plug(struct task_struct *tsk)
{
 ...
 if (plug)
  blk_flush_plug_list(plug, _true);
}
...
static inline int queue_limit_discard_alignment(struct queue_limits *lim, sector_t sector)
{
 ...
 offset = ( { int _res; _res = (sector) % (granularity); (sector) /= (granularity); _res; } );
...
void __attribute__ ((__section__(".meminit.text"))) __attribute__((__cold__)) pgdat_page_cgroup_init(struct pglist_data *pgdat);


static inline void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) page_cgroup_init_flatmem(void)
{
}
extern void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) page_cgroup_init(void);
...
static inline int PageCgroupUsed(struct page_cgroup *pc) { return (__builtin_constant_p((PCG_USED)) ? constant_test_bit((PCG_USED), (&pc->flags)) : variable_test_bit((PCG_USED), (&pc->flags))); }
static inline void ClearPageCgroupUsed(struct page_cgroup *pc) { clear_bit(PCG_USED, &pc->flags); }
static inline void SetPageCgroupUsed(struct page_cgroup *pc) { set_bit(PCG_USED, &pc->flags); }

static inline void SetPageCgroupMigration(struct page_cgroup *pc) { set_bit(PCG_MIGRATION, &pc->flags); }
static inline void ClearPageCgroupMigration(struct page_cgroup *pc) { clear_bit(PCG_MIGRATION, &pc->flags); }
static inline int PageCgroupMigration(struct page_cgroup *pc) { return (__builtin_constant_p((PCG_MIGRATION)) ? constant_test_bit((PCG_MIGRATION), (&pc->flags)) : variable_test_bit((PCG_MIGRATION), (&pc->flags))); }
...
extern unsigned short swap_cgroup_cmpxchg(swp_entry_t ent,
     unsigned short old, unsigned short _new);
