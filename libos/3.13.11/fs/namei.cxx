...
static inline int fsnotify_inode_watches_children(struct inode *inode)
{
 ...
 return inode->i_fsnotify_mask & (0x00000001 | 0x00000002 | 0x00000004 | 0x00000008 | 0x00000010 | 0x00000020 | 0x00000040 | 0x00000080 | 0x00000100 | 0x00000200 | 0x00010000 | 0x00020000);
}
...
extern struct fsnotify_group *fsnotify_alloc_group( struct fsnotify_ops *ops);
...
extern void fsnotify_duplicate_mark(struct fsnotify_mark *_new, struct fsnotify_mark *old);
...
static inline int fsnotify_perm(struct file *file, int mask)
{
 ...
 ret = fsnotify_parent(path, 0, fsnotify_mask);
 if (ret)
  return ret;

 return fsnotify(inode, fsnotify_mask, path, 1, 0, 0);
}
...
{
 fsnotify(inode, 0x00000004, inode, 2, 0, 0);
}
     ...
     int isdir, struct inode *target, struct dentry *moved)
{
 ...
 unsigned char *new_name = moved->d_name.name;
 ...
 if (source)
  fsnotify(source, 0x00000800, moved->d_inode, 2, 0, 0);
...
{
 fsnotify(inode, 0x00000400, inode, 2, 0, 0);
 __fsnotify_inode_delete(inode);
}
...
static inline void fsnotify_access(struct file *file)
{
 ...
 if (!(file->f_mode & (( fmode_t)0x1000000))) {
  fsnotify_parent(path, 0, mask);
  fsnotify(inode, mask, path, 1, 0, 0);
 }
}
...
static inline void fsnotify_xattr(struct dentry *dentry)
{
 ...
 fsnotify_parent(0, dentry, mask);
 fsnotify(inode, mask, inode, 2, 0, 0);
}
...
static inline unsigned char *fsnotify_oldname_init( unsigned char *name)
{



 return kstrdup((char *)name, ((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u)));
...
static inline void fsnotify_oldname_free( unsigned char *old_name)
{
 kfree(old_name);
}
...
extern int ima_inode_setxattr(struct dentry *dentry, char *xattr_name,
         void *xattr_value, size_t xattr_value_len);
extern int ima_inode_removexattr(struct dentry *dentry, char *xattr_name);
struct epoll_event;
struct iattr;
struct inode;
struct iocb;
...
{
 if (__builtin_expect(!!(!inode->i_rdev), 1))
  return 0;
 if (!(((inode->i_mode) & 00170000) == 0060000) && !(((inode->i_mode) & 00170000) == 0020000))
  return 0;
...
extern int posix_acl_valid( struct posix_acl *);
extern int posix_acl_permission(struct inode *, struct posix_acl *, int);
extern struct posix_acl *posix_acl_from_mode(umode_t, gfp_t);
extern int posix_acl_equiv_mode( struct posix_acl *, umode_t *);
...
extern int user_path_mountpoint_at(int, char *, unsigned int, struct path *);
extern int vfs_path_lookup(struct dentry *, struct vfsmount *,
      char *, unsigned int, struct path *);




extern int copy_mount_options( void *, unsigned long *);
extern int copy_mount_string( void *, char **);
...
extern struct file *do_file_open_root(struct dentry *, struct vfsmount *,
  char *, struct open_flags *);

extern long do_handle_open(int mountdirfd,
      struct file_handle *ufh, int open_flag);
...
extern ssize_t __kernel_write(struct file *, char *, size_t, loff_t *);
extern int rw_verify_area(int, struct file *, loff_t *, size_t);




extern struct file_operations pipefifo_fops;
...
{
 return ({ typeof( ((struct mount *)0)->mnt ) *__mptr = (mnt); (struct mount *)( (char *)__mptr - ((size_t) &((struct mount *)0)->mnt) );});
}
...
static inline unsigned long has_zero(unsigned long a, unsigned long *bits, struct word_at_a_time *c)
{
...
static inline unsigned long prep_zero_mask(unsigned long a, unsigned long bits, struct word_at_a_time *c)
{
 return bits;
}
...
static inline unsigned long load_unaligned_zeropad( void *addr)
{
...
unsigned int full_name_hash( unsigned char *name, unsigned int len)
{
