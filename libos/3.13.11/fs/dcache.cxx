...
unsigned int d_hash_mask __attribute__((__section__(".data..read_mostly")));



extern

unsigned int d_hash_shift __attribute__((__section__(".data..read_mostly")));
...
struct hlist_bl_head *dentry_hashtable __attribute__((__section__(".data..read_mostly")));

static inline struct hlist_bl_head *d_hash( struct dentry *parent,
     unsigned int hash)
{
 hash += (unsigned long) parent / (1 << (6));
...
static inline int dentry_string_cmp( unsigned char *cs, unsigned char *ct, unsigned tcount)
{
 ...
 mask = (~(~0ul << (tcount)*8));
 return __builtin_expect(!!(!!((a ^ b) & mask)), 0);
}
static inline int dentry_cmp( struct dentry *dentry, unsigned char *ct, unsigned tcount)
{
 unsigned char *cs;
 cs = (dentry->d_name.name);
 do { } while (0);
...
struct dentry *__d_lookup( struct dentry *parent, struct qstr *name)
{
 ...
 for (node = hlist_bl_first_rcu(b); node && ({ dentry = ({ typeof( ((typeof(*dentry) *)0)->d_hash ) *__mptr = (node); (typeof(*dentry) *)( (char *)__mptr - ((size_t) &((typeof(*dentry) *)0)->d_hash) );}); 1; }); node = (node->next)) {
...
{
 char *dname = (name->name);



 u32 dlen = *(typeof(&name->len))g_map(&name->len, sizeof(*(&name->len)));
...
static int prepend_path( struct path *path,
   struct path *root,
...
char *__d_path( struct path *path,
        struct path *root,
        char *buf, int buflen)
{
 char *res = buf + buflen;
...
char *d_absolute_path( struct path *path,
        char *buf, int buflen)
{
 struct path root = {};
 char *res = buf + buflen;
...
static int path_with_deleted( struct path *path,
        struct path *root,
        char **buf, int *buflen)
{
...
char *d_path( struct path *path, char *buf, int buflen)
{
 ...
 struct path root;
 int error;
        struct dentry_operations *op = path->dentry->d_op;

 if (op && *(typeof(&op->d_dname))g_map(&op->d_dname, sizeof(*(&op->d_dname))) &&
     (!((path->dentry) == (path->dentry)->d_parent) || path->dentry != path->mnt->mnt_root))
                return dentry_op_dname(op, path->dentry, buf, buflen);


 ;
 get_fs_root_rcu(get_current()->fs, &root);
