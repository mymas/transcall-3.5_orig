...
static inline u32 jiffies_to_AHZ(unsigned long x)
{
        ...
        u64 tmp = (u64)x * ((1000000000L +250/2)/250);
...
{
 mnt->mnt.mnt_flags &= ~(0x2000);
 mnt->mnt.mnt_flags |= 0x1000;
}
...
int get_dominating_id(struct mount *mnt, struct path *root);
...
unsigned int m_hash_mask __attribute__((__section__(".data..read_mostly")));



extern

unsigned int m_hash_shift __attribute__((__section__(".data..read_mostly")));
extern

struct hlist_head *mount_hashtable __attribute__((__section__(".data..read_mostly")));
static inline struct hlist_head *m_hash(struct vfsmount *mnt, struct dentry *dentry)
{
 unsigned long tmp = ((unsigned long)mnt / (1 << (6)));
 tmp += ((unsigned long)dentry / (1 << (6)));
...

{
    ...
    printf("first: %p\n", first);
    if (first)
        p = (struct mount *)((char *)first - (size_t)&((struct mount *)0)->mnt_hash);
    else
        p = 0;
    printf("p: %p\n", p);

    while (p) {
        struct hlist_node *next = p->mnt_hash.next;
        printf("next: %p\n", next);
        if (next)
            p = (struct mount *)((char *)next - (size_t)&((struct mount *)0)->mnt_hash);
        else
            p = 0;
        printf("p: %p\n", p);
    }
 ...
 for (p = ({ typeof((((head)->first))) ____ptr = ((((head)->first))); ____ptr ? ({ typeof( ((typeof(*(p)) *)0)->mnt_hash ) *__mptr = (____ptr); (typeof(*(p)) *)( (char *)__mptr - ((size_t) &((typeof(*(p)) *)0)->mnt_hash) );}) : 0; }); p; p = ({ typeof((((&(p)->mnt_hash)->next))) ____ptr = ((((&(p)->mnt_hash)->next))); ____ptr ? ({ typeof( ((typeof(*(p)) *)0)->mnt_hash ) *__mptr = (____ptr); (typeof(*(p)) *)( (char *)__mptr - ((size_t) &((typeof(*(p)) *)0)->mnt_hash) );}) : 0; })) {



                struct mount *parent = p->mnt_parent;
                if (&parent->mnt == mnt && p->mnt_mountpoint == dentry)
