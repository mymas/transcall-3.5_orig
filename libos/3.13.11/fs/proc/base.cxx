...
{
 get_current()->ioac.read_bytes += bytes;
}
...
static inline unsigned long task_io_get_inblock( struct task_struct *p)
{
...
{
 get_current()->ioac.write_bytes += bytes;
}
...
static inline unsigned long task_io_get_oublock( struct task_struct *p)
{
...
{
 get_current()->ioac.cancelled_write_bytes += bytes;
}
...
static inline bool close_on_exec(int fd, struct fdtable *fdt)
{
 return (__builtin_constant_p((fd)) ? constant_test_bit((fd), (fdt->close_on_exec)) : variable_test_bit((fd), (fdt->close_on_exec)));
}

static inline bool fd_is_open(int fd, struct fdtable *fdt)
{
 return (__builtin_constant_p((fd)) ? constant_test_bit((fd), (fdt->open_fds)) : variable_test_bit((fd), (fdt->open_fds)));
}
...
extern int user_path_at_empty(int, char *, unsigned, struct path *, int *empty);
...
extern int kern_path( char *, unsigned, struct path *);

extern struct dentry *kern_path_create(int, char *, struct path *, unsigned int);
extern struct dentry *user_path_create(int, char *, struct path *, unsigned int);
extern void done_path_create(struct path *, struct dentry *);
extern struct dentry *kern_path_locked( char *, struct path *);
extern int kern_path_mountpoint(int, char *, struct path *, unsigned int);
...
static inline bool
retry_estale( long error, unsigned int flags)
{
 return error == -116 && !(flags & 0x0020);
}
...
extern struct file_operations proc_mounts_operations;
extern struct file_operations proc_mountinfo_operations;
extern struct file_operations proc_mountstats_operations;
...
static inline int tracehook_report_syscall_entry(
 struct pt_regs *regs)
{
...
{
 get_current()->signal->oom_flags |= (( oom_flags_t)0x1);
}
...
static inline bool oom_task_origin( struct task_struct *p)
{
 return !!(p->signal->oom_flags & (( oom_flags_t)0x1));
}
...
extern enum oom_scan_t oom_scan_process_thread(struct task_struct *task,
  unsigned long totalpages, nodemask_t *nodemask,
  bool force_kill);
...
{
 oom_killer_disabled = _true;
}

static inline void oom_killer_enable(void)
{
 oom_killer_disabled = _false;
}

static inline bool oom_gfp_allowed(gfp_t gfp_mask)
{
 return (gfp_mask & (( gfp_t)0x80u)) && !(gfp_mask & (( gfp_t)0x1000u));
}
...
static inline void trace_oom_score_adj_update(struct task_struct *task) { } static inline void trace_oom_score_adj_update_rcuidle(struct task_struct *task) { } static inline int register_trace_oom_score_adj_update(void (*probe)(void *__data, struct task_struct *task), void *data) { return -38; } static inline int unregister_trace_oom_score_adj_update(void (*probe)(void *__data, struct task_struct *task), void *data) { return -38; } static inline void check_trace_callback_type_oom_score_adj_update(void (*cb)(void *__data, struct task_struct *task)) { }
 ;
...
extern struct file_operations proc_fd_operations;
extern struct inode_operations proc_fd_inode_operations;

extern struct file_operations proc_fdinfo_operations;
extern struct inode_operations proc_fdinfo_inode_operations;
...
int proc_pid_cmdline(struct task_struct *task, char * buffer)
{
 ...
 if (res > 0 && buffer[res-1] != '\0' && len < ((1UL) << 12)) {
...
{
 struct mm_struct *mm = mm_access(task, 0x01);
 int res = PTR_ERR(mm);
 if (mm && !IS_ERR(mm)) {
...
int lstats_show_proc(struct seq_file *m, void *v)
{
 int i;
...
{
 char *tmp = (char*)__get_free_pages((((( gfp_t)0x10u) | (( gfp_t)0x40u) | (( gfp_t)0x80u) | (( gfp_t)0x80000u))), 0);
 char *pathname;
 int len;
 ...
 pathname = d_path(path, tmp, ((1UL) << 12));
 len = PTR_ERR(pathname);
 if (IS_ERR(pathname))
  goto out;
 len = tmp + ((1UL) << 12) - 1 - pathname;
