...
extern unsigned long hugetlb_zero, hugetlb_infinity;
...
struct file *hugetlb_file_setup( char *name, size_t size, vm_flags_t acct,
...
{
 return (unsigned long)((1UL) << 12) << h->order;
}
...
{
 return pfn_pte((unsigned long)((page) - ((struct page *)(0xffffea0000000000UL))), (pgprot));
}
...
{
 return size_to_hstate(((1UL) << 12) << compound_order(page));
}

static inline unsigned hstate_index_to_shift(unsigned index)
{
 return hstates[index].order + 12;
}
...
static inline unsigned long basepage_index(struct page *page)
{
...
{
 return pmd_huge_support() && (huge_page_shift(h) == 21);
}

static inline spinlock_t *huge_pte_lockptr(struct hstate *h,
        struct mm_struct *mm, pte_t *pte)
{
 if (huge_page_size(h) == ((1UL) << 21))
...
bool mm_match_cgroup( struct mm_struct *mm, struct mem_cgroup *memcg)
{
 ...
 ;
 task_memcg = mem_cgroup_from_task((mm->owner));
...
struct proc_ns {
 void *ns;
 struct proc_ns_operations *ns_ops;
};

extern struct proc_ns_operations netns_operations;
extern struct proc_ns_operations utsns_operations;
extern struct proc_ns_operations ipcns_operations;
extern struct proc_ns_operations pidns_operations;
extern struct proc_ns_operations userns_operations;
extern struct proc_ns_operations mntns_operations;
...
static inline void *__PDE_DATA( struct inode *inode)
{
...
extern struct file_operations proc_tid_children_operations;
...
extern int pid_delete_dentry( struct dentry *);
...
extern bool proc_fill_cache(struct file *, struct dir_context *, char *, int,
      instantiate_t, struct task_struct *, void *);
...
extern struct inode_operations proc_pid_link_inode_operations;

extern void proc_init_inodecache(void);
extern struct inode *proc_get_inode(struct super_block *, struct proc_dir_entry *);
extern int proc_fill_super(struct super_block *);
extern void proc_entry_rundown(struct proc_dir_entry *);
extern struct inode_operations proc_ns_dir_inode_operations;
extern struct file_operations proc_ns_dir_operations;




extern struct file_operations proc_net_operations;
...
extern struct file_operations proc_pid_maps_operations;
extern struct file_operations proc_tid_maps_operations;
extern struct file_operations proc_pid_numa_maps_operations;
extern struct file_operations proc_tid_numa_maps_operations;
extern struct file_operations proc_pid_smaps_operations;
extern struct file_operations proc_tid_smaps_operations;
...
int meminfo_proc_show(struct seq_file *m, void *v)
{
 ...
 for (lru = 0; lru < NR_LRU_LISTS; lru++)
                ...
                pages[lru] = global_page_state((enum zone_stat_item)(NR_LRU_BASE + lru));
 ...
 seq_printf(m,
  ...
  ((i.bufferram) << (12 - 10)),
  ((cached) << (12 - 10)),
  ((total_swapcache_pages()) << (12 - 10)),
  ((pages[LRU_ACTIVE_ANON] + pages[LRU_ACTIVE_FILE]) << (12 - 10)),
  ((pages[LRU_INACTIVE_ANON] + pages[LRU_INACTIVE_FILE]) << (12 - 10)),
  ((pages[LRU_ACTIVE_ANON]) << (12 - 10)),
  ((pages[LRU_INACTIVE_ANON]) << (12 - 10)),
  ((pages[LRU_ACTIVE_FILE]) << (12 - 10)),
  ((pages[LRU_INACTIVE_FILE]) << (12 - 10)),
  ((pages[LRU_UNEVICTABLE]) << (12 - 10)),
  ((global_page_state(NR_MLOCK)) << (12 - 10)),
  ((i.totalswap) << (12 - 10)),
  ((i.freeswap) << (12 - 10)),
  ((global_page_state(NR_FILE_DIRTY)) << (12 - 10)),
  ((global_page_state(NR_WRITEBACK)) << (12 - 10)),
  ((global_page_state(NR_ANON_PAGES)) << (12 - 10)),
  ((global_page_state(NR_FILE_MAPPED)) << (12 - 10)),
  ((global_page_state(NR_SHMEM)) << (12 - 10)),
  ((global_page_state(NR_SLAB_RECLAIMABLE) + global_page_state(NR_SLAB_UNRECLAIMABLE)) << (12 - 10))
                                             ,
  ((global_page_state(NR_SLAB_RECLAIMABLE)) << (12 - 10)),
  ((global_page_state(NR_SLAB_UNRECLAIMABLE)) << (12 - 10)),
  global_page_state(NR_KERNEL_STACK) * (((1UL) << 12) << 1) / 1024,
  ((global_page_state(NR_PAGETABLE)) << (12 - 10)),



  ((global_page_state(NR_UNSTABLE_NFS)) << (12 - 10)),
  ((global_page_state(NR_BOUNCE)) << (12 - 10)),
  ((global_page_state(NR_WRITEBACK_TEMP)) << (12 - 10)),
  ((vm_commit_limit()) << (12 - 10)),
  ((committed) << (12 - 10)),
  (unsigned long)((0xffffe8ffffffffffUL) - (0xffffc90000000000UL)) >> 10,
  vmi.used >> 10,
  vmi.largest_chunk >> 10

  ,atomic_long_read(&num_poisoned_pages) << (12 - 10)


  ,((global_page_state(NR_ANON_TRANSPARENT_HUGEPAGES) * (1<<(21 -12))) << (12 - 10))
