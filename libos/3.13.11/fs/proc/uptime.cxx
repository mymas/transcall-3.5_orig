...
int uptime_proc_show(struct seq_file *m, void *v)
{
 ...
 for (((i)) = -1; ((i)) = cpumask_next(((i)), (cpu_possible_mask)), ((i)) < nr_cpu_ids;)
  idletime += ( u64) (*({ do { void *__vpp_verify = (typeof(((&(kernel_cpustat))) + 0))0; (void)__vpp_verify; } while (0); ({ unsigned long __ptr; __asm__ ("" : "=r"(__ptr) : "0"((typeof(*(&(kernel_cpustat))) *)(&(kernel_cpustat)))); (typeof((typeof(*(&(kernel_cpustat))) *)(&(kernel_cpustat)))) (__ptr + (((__per_cpu_offset[i])))); }); })).cpustat[CPUTIME_IDLE];

 get_monotonic_boottime(&uptime);
 nsec = div_u64(( u64)idletime, 1000000000L / 250) * ((1000000000L +250/2)/250);
 idle.tv_sec = div_u64_rem(nsec, 1000000000L, &rem);
 idle.tv_nsec = rem;
