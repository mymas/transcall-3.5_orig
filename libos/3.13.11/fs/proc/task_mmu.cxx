     ...
     spinlock_t **ptlp, int sync)
{
 ...
 (ptep = __page_check_address(page, mm, address, ptlp, sync))
                         ;
 return ptep;
}
...
static inline swp_entry_t swp_entry(unsigned long type, unsigned long offset)
{
 ...
 ret.val = (type << ((sizeof(ret.val) * 8) - (5 + 2))) |
   (offset & ((1UL << ((sizeof(ret.val) * 8) - (5 + 2))) - 1));
 return ret;
}
...
{
 return (entry.val >> ((sizeof(entry.val) * 8) - (5 + 2)));
}
...
{
 return entry.val & ((1UL << ((sizeof(entry.val) * 8) - (5 + 2))) - 1);
}
...
static inline pte_t swp_entry_to_pte(swp_entry_t entry)
{
 ...
 (pte_file(({ pte_t var; var.pte = (arch_entry).val; var; })));
 return ({ pte_t var; var.pte = (arch_entry).val; var; });
}
...
void task_mem(struct seq_file *m, struct mm_struct *mm)
{
 ...
 text = (((((mm->end_code)) + ((typeof((mm->end_code)))((((1UL) << 12))) - 1)) & ~((typeof((mm->end_code)))((((1UL) << 12))) - 1)) - (mm->start_code & (~(((1UL) << 12)-1)))) >> 10;
 lib = (mm->exec_vm << (12 -10)) - text;
 ...
 *text = (((((mm->end_code)) + ((typeof((mm->end_code)))((((1UL) << 12))) - 1)) & ~((typeof((mm->end_code)))((((1UL) << 12))) - 1)) - (mm->start_code & (~(((1UL) << 12)-1))))
        >> 12;
...
static void *m_start(struct seq_file *m, loff_t *pos)
{
 ...
 struct mm_struct *mm;
 struct vm_area_struct *vma, *tail_vma = 0;
 loff_t l = *pos;
...
static void *m_next(struct seq_file *m, void *v, loff_t *pos)
{
 ...
 if (vma && (vma != tail_vma) && vma->vm_next)
  return vma->vm_next;
 vma_stop(priv, vma);
 return (vma != tail_vma)? tail_vma: 0;
}
...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma, int is_pid)
{
 ...
 if (file) {
  struct inode *inode;
  file = vma_do_pr_or_file(vma, (char *)__func__, 272);
  inode = file_inode(file);
  dev = inode->i_sb->s_dev;
  ino = inode->i_ino;
  pgoff = ((loff_t)vma->vm_pgoff) << 12;
 }
 ...
 seq_printf(m, "%08lx-%08lx %c%c%c%c %08llx %02x:%02x %lu ",
   ...
   flags & 0x00000002 ? 'w' : '-',
   flags & 0x00000004 ? 'x' : '-',
   flags & 0x00000080 ? 's' : 'p',
   pgoff,
   ((unsigned int) ((dev) >> 20)), ((unsigned int) ((dev) & ((1U << 20) - 1))), ino);
...
struct seq_operations proc_tid_maps_op = {
    m_start,
    m_stop,
    m_next,
    show_tid_map
