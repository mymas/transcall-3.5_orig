...
void generic_fillattr(struct inode *inode, struct kstat *stat)
{
 ...
 stat->nlink = *(typeof(&inode->i_nlink))g_map(&inode->i_nlink, sizeof(*(&inode->i_nlink)));
...
int vfs_getattr_nosec(struct path *path, struct kstat *stat)
{
 struct inode *inode = path->dentry->d_inode;
        ...
        if (inode_op_getattr(inode->i_op, path->mnt, path->dentry, stat) == 0)
            return 0;
...
int cp_new_stat(struct kstat *stat, struct stat *statbuf)
{
 ...
 do { tmp.__pad0 = 0; tmp.__unused[0] = 0; tmp.__unused[1] = 0; tmp.__unused[2] = 0; } while (0);
 ...
 if (tmp.st_nlink != stat->nlink)
  return -75;
 do { (tmp.st_uid) = (sizeof(tmp.st_uid) >= sizeof((from_kuid_munged((({ (get_current()->cred)->user_ns; })), stat->uid))) ? ((from_kuid_munged((({ (get_current()->cred)->user_ns; })), stat->uid))) : (((from_kuid_munged((({ (get_current()->cred)->user_ns; })), stat->uid))) & ~0xFFFF ? (old_uid_t)overflowuid : (old_uid_t)((from_kuid_munged((({ (get_current()->cred)->user_ns; })), stat->uid))))); } while (0);
 do { (tmp.st_gid) = (sizeof(tmp.st_gid) >= sizeof((from_kgid_munged((({ (get_current()->cred)->user_ns; })), stat->gid))) ? ((from_kgid_munged((({ (get_current()->cred)->user_ns; })), stat->gid))) : (((from_kgid_munged((({ (get_current()->cred)->user_ns; })), stat->gid))) & ~0xFFFF ? (old_gid_t)overflowgid : (old_gid_t)((from_kgid_munged((({ (get_current()->cred)->user_ns; })), stat->gid))))); } while (0);
 ...
 return copy_to_user(statbuf,&tmp,sizeof(tmp)) ? -14 : 0;
}
