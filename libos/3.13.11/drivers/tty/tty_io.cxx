      ...
      struct termio *termio)
{
 ...
 if (({ int __ret_gu; register __typeof__(0ULL) __val_gu asm("%""rdx"); (void)0; might_fault(); asm ("call __get_user_%P3" : "=a" (__ret_gu), "=r" (__val_gu) : "0" (&termio->c_iflag), "i" (sizeof(*(&termio->c_iflag)))); (tmp) = (__typeof__(*(&termio->c_iflag))) __val_gu; __ret_gu; }) < 0)
  goto fault;
 termios->c_iflag = (0xffff0000 & termios->c_iflag) | tmp;

 if (({ int __ret_gu; register __typeof__(0ULL) __val_gu asm("%""rdx"); (void)0; might_fault(); asm ("call __get_user_%P3" : "=a" (__ret_gu), "=r" (__val_gu) : "0" (&termio->c_oflag), "i" (sizeof(*(&termio->c_oflag)))); (tmp) = (__typeof__(*(&termio->c_oflag))) __val_gu; __ret_gu; }) < 0)
  goto fault;
 termios->c_oflag = (0xffff0000 & termios->c_oflag) | tmp;

 if (({ int __ret_gu; register __typeof__(0ULL) __val_gu asm("%""rdx"); (void)0; might_fault(); asm ("call __get_user_%P3" : "=a" (__ret_gu), "=r" (__val_gu) : "0" (&termio->c_cflag), "i" (sizeof(*(&termio->c_cflag)))); (tmp) = (__typeof__(*(&termio->c_cflag))) __val_gu; __ret_gu; }) < 0)
  goto fault;
 termios->c_cflag = (0xffff0000 & termios->c_cflag) | tmp;

 if (({ int __ret_gu; register __typeof__(0ULL) __val_gu asm("%""rdx"); (void)0; might_fault(); asm ("call __get_user_%P3" : "=a" (__ret_gu), "=r" (__val_gu) : "0" (&termio->c_lflag), "i" (sizeof(*(&termio->c_lflag)))); (tmp) = (__typeof__(*(&termio->c_lflag))) __val_gu; __ret_gu; }) < 0)
  goto fault;
 termios->c_lflag = (0xffff0000 & termios->c_lflag) | tmp;

 if (({ int __ret_gu; register __typeof__(0ULL) __val_gu asm("%""rdx"); (void)0; might_fault(); asm ("call __get_user_%P3" : "=a" (__ret_gu), "=r" (__val_gu) : "0" (&termio->c_line), "i" (sizeof(*(&termio->c_line)))); (termios->c_line) = (__typeof__(*(&termio->c_line))) __val_gu; __ret_gu; }) < 0)
  goto fault;
...
{
 if (({ __typeof__(*(&termio->c_iflag)) __x = (termios->c_iflag); copy_to_user((void *)&termio->c_iflag, (void *)&__x, sizeof(*(&termio->c_iflag))); }) < 0 ||
     ({ __typeof__(*(&termio->c_oflag)) __x = (termios->c_oflag); copy_to_user((void *)&termio->c_oflag, (void *)&__x, sizeof(*(&termio->c_oflag))); }) < 0 ||
     ({ __typeof__(*(&termio->c_cflag)) __x = (termios->c_cflag); copy_to_user((void *)&termio->c_cflag, (void *)&__x, sizeof(*(&termio->c_cflag))); }) < 0 ||
     ({ __typeof__(*(&termio->c_lflag)) __x = (termios->c_lflag); copy_to_user((void *)&termio->c_lflag, (void *)&__x, sizeof(*(&termio->c_lflag))); }) < 0 ||
     ({ __typeof__(*(&termio->c_line)) __x = (termios->c_line); copy_to_user((void *)&termio->c_line, (void *)&__x, sizeof(*(&termio->c_line))); }) < 0 ||
...
extern void __init_ldsem(struct ld_semaphore *sem, char *name,
    struct lock_class_key *key);
...
struct tty_ldisc_ops {
 ...
 int (*receive_buf2)(struct tty_struct *, unsigned char *cp,
    char *fp, int count);
...
extern struct _class *tty_class;
     ...
     char *f, int count)
{
 if (ld->ops->receive_buf2)
  ...
  count = ({ int __min1 = (count); int __min2 = (ld->tty->receive_room); __min1 < __min2 ? __min1: __min2; });
  if (count)
...
extern void __attribute__((section(".spinlock.text"))) tty_lock(struct tty_struct *tty);
extern void __attribute__((section(".spinlock.text"))) tty_unlock(struct tty_struct *tty);
extern void __attribute__((section(".spinlock.text"))) tty_lock_pair(struct tty_struct *tty,
    struct tty_struct *tty2);
extern void __attribute__((section(".spinlock.text"))) tty_unlock_pair(struct tty_struct *tty,
    struct tty_struct *tty2);
...
extern int tty_insert_flip_string_fixed_flag(struct tty_port *port,
  unsigned char *chars, char flag, size_t size);
...
static inline int tty_insert_flip_string(struct tty_port *port,
  unsigned char *chars, size_t size)
{
 return tty_insert_flip_string_fixed_flag(port, chars, 0, size);
}
...
struct consw {
 ...
 void (*con_putcs)(struct vc_data *, unsigned short *, int, int, int);
...
int con_is_bound( struct consw *csw);
int do_unregister_con_driver( struct consw *csw);
int do_take_over_console( struct consw *sw, int first, int last, int deflt);
...
struct console {
 char name[16];
 void (*write)(struct console *, char *, unsigned);
...
extern int vt_kmsg_redirect(int _new);
...
void vt_event_post(unsigned int event, unsigned int old, unsigned int _new);
int vt_waitactive(int n);
void change_console(struct vc_data *new_vc);
void reset_vc(struct vc_data *vc);
extern int do_unbind_con_driver( struct consw *csw, int first, int last,
        int deflt);
int vty_init( struct file_operations *console_fops);
...
extern int vt_do_diacrit(unsigned int cmd, void *up, int eperm);
extern int vt_do_kdskbmode(int console, unsigned int arg);
extern int vt_do_kdskbmeta(int console, unsigned int arg);
extern int vt_do_kbkeycode_ioctl(int cmd, struct kbkeycode *user_kbkc,
        int perm);
extern int vt_do_kdsk_ioctl(int cmd, struct kbentry *user_kbe,
     int perm, int console);
extern int vt_do_kdgkb_ioctl(int cmd, struct kbsentry *user_kdgkb,
                                        int perm);
...
extern int set_selection( struct tiocl_selection *sel, struct tty_struct *tty);
...
extern u16 vcs_scr_readw(struct vc_data *vc, u16 *org);
