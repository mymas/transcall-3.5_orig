...
extern typeof(generic_fillattr) generic_fillattr; static char __kstrtab_generic_fillattr[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "generic_fillattr"; static struct kernel_symbol __ksymtab_generic_fillattr __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&generic_fillattr, __kstrtab_generic_fillattr };
...
extern typeof(vfs_getattr) vfs_getattr; static char __kstrtab_vfs_getattr[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "vfs_getattr"; static struct kernel_symbol __ksymtab_vfs_getattr __attribute__((__used__)) __attribute__((section("__ksymtab" ""), unused)) = { (unsigned long)&vfs_getattr, __kstrtab_vfs_getattr };
int cp_new_stat(struct kstat *stat, struct stat *statbuf)
{
 ...
 if (tmp.st_nlink != stat->nlink)
  return -75;
 do { (tmp.st_uid) = (sizeof(tmp.st_uid) >= sizeof((stat->uid)) ? ((stat->uid)) : (((stat->uid)) & ~0xFFFF ? (old_uid_t)overflowuid : (old_uid_t)((stat->uid)))); } while (0);
 do { (tmp.st_gid) = (sizeof(tmp.st_gid) >= sizeof((stat->gid)) ? ((stat->gid)) : (((stat->gid)) & ~0xFFFF ? (old_gid_t)overflowgid : (old_gid_t)((stat->gid)))); } while (0);
