...
extern struct inotify_handle *inotify_init( struct inotify_operations *);
     ...
     int isdir, struct inode *target, struct dentry *moved)
{
 ...
 if (old_dir == new_dir)
  inode_dir_notify(old_dir, 0x00000010);
 else {
  inode_dir_notify(old_dir, 0x00000008);
  inode_dir_notify(new_dir, 0x00000004);
 }

 if (isdir)
  isdir = 0x40000000;
 inotify_inode_queue_event(old_dir, 0x00000040|isdir,cookie,old_name,
      source);
 inotify_inode_queue_event(new_dir, 0x00000080|isdir, cookie, new_name,
      source);

 if (target) {
  inotify_inode_queue_event(target, 0x00000400, 0, 0, 0);
  inotify_inode_is_dead(target);
 }

 if (source) {
  inotify_inode_queue_event(source, 0x00000800, 0, 0, 0);
 }
...
static inline void fsnotify_nameremove(struct dentry *dentry, int isdir)
{
 if (isdir)
  isdir = 0x40000000;
 dnotify_parent(dentry, 0x00000008);
 inotify_dentry_parent_queue_event(dentry, (__u32)0x00000200|isdir, 0, (char *)dentry->d_name.name);
}
...
{
 inotify_inode_queue_event(inode, 0x00000400, 0, 0, 0);
...
{
 inotify_inode_queue_event(inode, 0x00000004, 0, 0, 0);
}
...
{
 inode_dir_notify(inode, 0x00000004);
 inotify_inode_queue_event(inode, 0x00000100, 0, (char *)dentry->d_name.name,
      dentry->d_inode);
 audit_inode_child((char *)dentry->d_name.name, dentry, inode);
}
...
{
 inode_dir_notify(dir, 0x00000004);
 inotify_inode_queue_event(dir, 0x00000100, 0, (char *)new_dentry->d_name.name,
      inode);
 fsnotify_link_count(inode);
 audit_inode_child((char *)new_dentry->d_name.name, new_dentry, dir);
}
...
{
 inode_dir_notify(inode, 0x00000004);
 inotify_inode_queue_event(inode, 0x00000100 | 0x40000000, 0,
      (char *)dentry->d_name.name, dentry->d_inode);
 audit_inode_child((char *)dentry->d_name.name, dentry, inode);
}
...
static inline void fsnotify_access(struct dentry *dentry)
{
 ...
 dnotify_parent(dentry, 0x00000001);
 inotify_dentry_parent_queue_event(dentry, mask, 0, (char *)dentry->d_name.name);
 inotify_inode_queue_event(inode, mask, 0, 0, 0);
}
...
static inline void fsnotify_modify(struct dentry *dentry)
{
 ...
 dnotify_parent(dentry, 0x00000002);
 inotify_dentry_parent_queue_event(dentry, mask, 0, (char *)dentry->d_name.name);
 inotify_inode_queue_event(inode, mask, 0, 0, 0);
}
...
static inline void fsnotify_close(struct file *file)
{
 ...
 u32 mask = (mode & (2)) ? 0x00000008 : 0x00000010;
...
static inline void fsnotify_change(struct dentry *dentry, unsigned int ia_valid)
{
 ...
 if (in_mask) {
  if ((((inode->i_mode) & 00170000) == 0040000))
   in_mask |= 0x40000000;
  inotify_inode_queue_event(inode, in_mask, 0, 0, 0);
...
static inline char *fsnotify_oldname_init( char *name)
{
...
static inline void fsnotify_oldname_free( char *old_name)
{
 kfree(old_name);
}
...
struct hlist_head *dentry_hashtable __attribute__((__section__(".data.read_mostly")));
static inline struct hlist_head *d_hash(struct dentry *parent,
     unsigned long hash)
{
 hash += ((unsigned long) parent ^ 0x9e37fffffffc0001UL) / (1 << (7));
 hash = hash ^ ((hash ^ 0x9e37fffffffc0001UL) >> d_hash_shift);
...
struct dentry * __d_lookup(struct dentry * parent, struct qstr * name)
{
 ...
 for (node = ((head)->first); node && ({ __builtin_prefetch(node->next); 1; }) && ({ dentry = ({ typeof( ((typeof(*dentry) *)0)->d_hash ) *__mptr = (node); (typeof(*dentry) *)( (char *)__mptr - ((size_t) &((typeof(*dentry) *)0)->d_hash) );}); 1; }); node = (node->next)) {
  struct qstr *qstr;
...
{
 return prepend(buffer, buflen, (char *)name->name, (int)name->len);
}
char *__d_path( struct path *path, struct path *root,
        char *buffer, int buflen)
{
 ...
 if (!((dentry) == (dentry)->d_parent) && d_unhashed(dentry) &&
...
char *d_path( struct path *path, char *buf, int buflen)
{
 ...
 struct path tmp;
 if (path->dentry->d_op) {
                res = dentry_op_dname(path->dentry->d_op, path->dentry, buf, buflen);
                if (res)
                        return res;
        }
