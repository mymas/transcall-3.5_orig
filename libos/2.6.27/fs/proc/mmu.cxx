...
void get_vmalloc_info(struct vmalloc_info *vmi)
{
 ...
 if (!vmlist) {
  vmi->largest_chunk = ((0xffffe1ffffffffffUL) - (0xffffc20000000000UL));
 }
 else {
  vmi->largest_chunk = 0;

  prev_end = (0xffffc20000000000UL);
  ...
  for (vma = vmlist; vma; vma = vma->next) {
   ...
   if (addr < (0xffffc20000000000UL))
    continue;
   if (addr >= (0xffffe1ffffffffffUL))
    break;
  ...
  if ((0xffffe1ffffffffffUL) - prev_end > vmi->largest_chunk)
   vmi->largest_chunk = (0xffffe1ffffffffffUL) - prev_end;
