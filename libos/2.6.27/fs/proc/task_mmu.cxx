...
{
 return !(!pte_val((pte))) && !(pte_val((pte)) & ((((pteval_t)(1)) << 0) | (((pteval_t)(1)) << 7))) && !pte_file(pte);
}
...
static inline swp_entry_t pte_to_swp_entry(pte_t pte)
{
 ...
 return swp_entry((((arch_entry).val >> 1) & 0x3f), ((arch_entry).val >> 8));
}
...
static inline pte_t swp_entry_to_pte(swp_entry_t entry)
{
 ...
 arch_entry = ((swp_entry_t) { ((swp_type(entry)) << 1) | ((swp_offset(entry)) << 8) });
 ;
...
{
 ;
 return swp_entry(write ? (((1 << 5)-2) + 1) : ((1 << 5)-2),
   ((page) - ((struct page *)(0xffffe20000000000UL))));
}

static inline int is_migration_entry(swp_entry_t entry)
{
 return __builtin_expect(!!(swp_type(entry) == ((1 << 5)-2) || swp_type(entry) == (((1 << 5)-2) + 1)), 0)
                                          ;
}

static inline int is_write_migration_entry(swp_entry_t entry)
{
 return __builtin_expect(!!(swp_type(entry) == (((1 << 5)-2) + 1)), 0);
}

static inline struct page *migration_entry_to_page(swp_entry_t entry)
{
 struct page *p = (((struct page *)(0xffffe20000000000UL)) + (swp_offset(entry)));
...
{
 *entry = swp_entry(((1 << 5)-2), swp_offset(*entry));
}
...
void task_mem(struct seq_file *m, struct mm_struct *mm)
{
 ...
 hiwater_rss = total_rss = (((unsigned long)atomic_long_read(&(mm)->_file_rss)) + ((unsigned long)atomic_long_read(&(mm)->_anon_rss)));
 ...
 seq_printf(m,
  ...
  "VmPTE:\t%8lu kB\n",
  hiwater_vm << (12 -10),
  (total_vm - mm->reserved_vm) << (12 -10),
  mm->locked_vm << (12 -10),
  hiwater_rss << (12 -10),
  total_rss << (12 -10),
  data << (12 -10),
  mm->stack_vm << (12 -10), text, lib,
  (512*sizeof(pte_t)*mm->nr_ptes) >> 10);
}
...
{
 *shared = ((unsigned long)atomic_long_read(&(mm)->_file_rss));
 *text = ((((mm->end_code)+((typeof(mm->end_code))(((1UL) << 12))-1))&~((typeof(mm->end_code))(((1UL) << 12))-1)) - (mm->start_code & (~(((1UL) << 12)-1))))
        >> 12;
 *data = mm->total_vm - mm->shared_vm;
 *resident = *shared + ((unsigned long)atomic_long_read(&(mm)->_anon_rss));
 return mm->total_vm;
}
...
static void show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
 ...
 seq_printf(m, "%08lx-%08lx %c%c%c%c %08llx %02x:%02x %lu %n",
   ...
   ((loff_t)vma->vm_pgoff) << 12,
   ((unsigned int) ((dev) >> 20)), ((unsigned int) ((dev) & ((1U << 20) - 1))), ino, &len);
