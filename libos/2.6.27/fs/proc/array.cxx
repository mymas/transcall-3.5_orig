...
{
 unsigned int state = (tsk->state & (0 | 1 | 2 | 4 | 8)) | tsk->exit_state;
    ...
    struct pid *pid, struct task_struct *p)
{
 ...
 ;
 ppid = pid_alive(p) ?
  task_tgid_nr_ns((p->real_parent), ns) : 0;
 tpid = 0;
 if (pid_alive(p)) {
 ...
 for (g = 0; g < ({ typeof(group_info->ngroups) _min1 = (group_info->ngroups); typeof(32) _min2 = (32); (void) (&_min1 == &_min2); _min1 < _min2 ? _min1 : _min2; }); g++)
  seq_printf(m, "%d ", ((group_info)->blocks[(g)/((unsigned int)(((1UL) << 12) / sizeof(gid_t)))][(g)%((unsigned int)(((1UL) << 12) / sizeof(gid_t)))]));
 do { if (1) ; } while (0);

 seq_printf(m, "\n");
}

static void render_sigset_t(struct seq_file *m, char *header,
    sigset_t *set)
{
 int i;
        ...
        sigset_t *_catch)
{
 ...
 k = p->sighand->action;
 for (i = 1; i <= 64; ++i, ++k) {
  if (k->sa.sa_handler == (( __sighandler_t)1))
   sigaddset(ign, i);
  else if (k->sa.sa_handler != (( __sighandler_t)0))
   sigaddset(_catch, i);
 }
}
...
static inline void task_sig(struct seq_file *m, struct task_struct *p)
{
 ...
 if ((p)) {
  ...
  num_threads = ((&(p->signal->count))->counter);
  qsize = ((&(p->user->sigpending))->counter);
  qlim = p->signal->rlim[11].rlim_cur;
  ;
 }
 ;
...
static void render_cap_t(struct seq_file *m, char *header,
   kernel_cap_t *a)
{
 unsigned __capi;
   ...
   struct pid *pid, struct task_struct *task, int whole)
{
 ...
 if (mm) {
  vsize = task_vsize(mm);
  if (permitted) {
   eip = (((struct pt_regs *)(task)->thread.sp0 - 1)->ip);
   esp = -1;
  }
 }
 ...
 if ((task)) {
  ...
  if (whole) {
   ...
   do {
    min_flt += t->min_flt;
    maj_flt += t->maj_flt;
    utime = ((utime) + (task_utime(t)));
    stime = ((stime) + (task_stime(t)));
    gtime = ((gtime) + (task_gtime(t)));
    t = next_thread(t);
   } while (t != task);

   min_flt += sig->min_flt;
   maj_flt += sig->maj_flt;
   utime = ((utime) + (sig->utime));
   stime = ((stime) + (sig->stime));
   gtime = ((gtime) + (sig->gtime));
  }
 ...
 start_time =
  (unsigned long long)task->real_start_time.tv_sec * 1000000000L
 ...
 seq_printf(m, "%d (%s) %c %d %d %d %d %d %u %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %d 0 %llu %lu %ld %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %d %d %u %u %llu %lu %ld\n",
  ...
  start_time,
  vsize,
  mm ? (((unsigned long)atomic_long_read(&(mm)->_file_rss)) + ((unsigned long)atomic_long_read(&(mm)->_anon_rss))) : 0,
  rsslim,
  mm ? mm->start_code : 0,
  mm ? mm->end_code : 0,
  ...
  jiffies_to_clock_t(gtime),
  jiffies_to_clock_t(cgtime));
 if (mm)
  mmput(mm);
 return 0;
}
