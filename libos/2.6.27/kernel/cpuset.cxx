...
void cpuset_task_status_allowed(struct seq_file *m, struct task_struct *task)
{
 ...
 m->count += __cpumask_scnprintf((m->buf + m->count), (m->size - m->count), &(task->cpus_allowed), 64)
                        ;
 seq_printf(m, "\n");
 seq_printf(m, "Cpus_allowed_list:\t");
 m->count += __cpulist_scnprintf((m->buf + m->count), (m->size - m->count), &(task->cpus_allowed), 64)
                        ;
 seq_printf(m, "\n");
 seq_printf(m, "Mems_allowed:\t");
 m->count += __nodemask_scnprintf((m->buf + m->count), (m->size - m->count), &(task->mems_allowed), (1 << 9))
                        ;
 seq_printf(m, "\n");
 seq_printf(m, "Mems_allowed_list:\t");
 m->count += __nodelist_scnprintf((m->buf + m->count), (m->size - m->count), &(task->mems_allowed), (1 << 9))
                        ;
 seq_printf(m, "\n");
}
