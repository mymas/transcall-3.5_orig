...
extern int eth_header_parse( struct sk_buff *skb, unsigned char *haddr);
...
static inline int is_zero_ether_addr( u8 *addr)
{
 return !(addr[0] | addr[1] | addr[2] | addr[3] | addr[4] | addr[5]);
}
static inline int is_multicast_ether_addr( u8 *addr)
{
 return (0x01 & addr[0]);
}
...
static inline int is_local_ether_addr( u8 *addr)
{
 return (0x02 & addr[0]);
}
...
static inline int is_broadcast_ether_addr( u8 *addr)
{
...
struct Qdisc_class_ops
{
 ...
 int (*_delete)(struct Qdisc *, unsigned long);
...
struct tcf_proto_ops
{
 ...
 int (*change)(struct tcf_proto*, unsigned long,
     ...
     unsigned long *);
 int (*_delete)(struct tcf_proto*, unsigned long);
...
static inline spinlock_t *qdisc_root_lock(struct Qdisc *qdisc)
{
 ...
 do { if (__builtin_expect(!!(!0), 0)) { printk("<3>" "RTNL: assertion failed at %s (%d)\n", "../../libos/2.6.27/include/net/sch_generic.h", 216); dump_stack(); } } while(0);
...
static inline spinlock_t *qdisc_root_sleeping_lock(struct Qdisc *qdisc)
{
 ...
 do { if (__builtin_expect(!!(!0), 0)) { printk("<3>" "RTNL: assertion failed at %s (%d)\n", "../../libos/2.6.27/include/net/sch_generic.h", 224); dump_stack(); } } while(0);
 return qdisc_lock(root);
}
...
qdisc_class_find(struct Qdisc_class_hash *hash, u32 id)
{
 ...
 for (n = (&hash->hash[h])->first; n && ({ __builtin_prefetch(n->next); 1;}) && ({ cl = ({ typeof( ((typeof(*cl) *)0)->hnode ) *__mptr = (n); (typeof(*cl) *)( (char *)__mptr - ((size_t) &((typeof(*cl) *)0)->hnode) );}); 1;}); n = n->next) {
  if (cl->classid == id)
   return cl;
 }
 return 0;
...
static inline bool qdisc_all_tx_empty( struct net_device *dev)
{
 unsigned int i;
 for (i = 0; i < dev->num_tx_queues; i++) {
  struct netdev_queue *txq = netdev_get_tx_queue(dev, i);
...
static inline int qdisc_reshape_fail(struct sk_buff *skb, struct Qdisc *sch)
{
 ...
 if (sch->reshape_fail == 0 || sch->reshape_fail(skb, sch))
  goto drop;
...
void netpoll_send_udp(struct netpoll *np, char *msg, int len);
...
static inline void *netpoll_poll_lock(struct napi_struct *napi)
{
 ...
 if (dev && dev->npinfo) {
  ;
  napi->poll_owner = ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; });
  return napi;
 }
 return 0;
}
             ...
             u16 vlan_id)
{
 ...
 return array ? array[vlan_id % (4096/8)] : 0;
}
...
static inline struct sk_buff *__vlan_put_tag(struct sk_buff *skb, u16 vlan_tci)
{
 ...
 veth->h_vlan_TCI = (( __be16)__fswab16((vlan_tci)));
 ...
 *vlan_tci = __fswab16(( __u16)(__be16)(veth->h_vlan_TCI));
 return 0;
}
...
static inline struct hlist_head *dev_name_hash(struct net *net, char *name)
{
 unsigned hash = full_name_hash((unsigned char *)name, strnlen(name, 16));
...
struct net_device *__dev_get_by_name(struct net *net, char *name)
{
 ...
 for (p = (dev_name_hash(net, name))->first; p && ({ __builtin_prefetch(p->next); 1; }); p = p->next) {
  struct net_device *dev
   = ({ typeof( ((struct net_device *)0)->name_hlist ) *__mptr = (p); (struct net_device *)( (char *)__mptr - ((size_t) &((struct net_device *)0)->name_hlist) );});
...
static gifconf_func_t * gifconf_list [35];
...
static int dev_ifconf(struct net *net, char *arg)
{
 ...
 total = 0;
 for (dev = ({ typeof( ((typeof(*dev) *)0)->dev_list ) *__mptr = ((&(net)->dev_base_head)->next); (typeof(*dev) *)( (char *)__mptr - ((size_t) &((typeof(*dev) *)0)->dev_list) );}); __builtin_prefetch(dev->dev_list.next), &dev->dev_list != (&(net)->dev_base_head); dev = ({ typeof( ((typeof(*dev) *)0)->dev_list ) *__mptr = (dev->dev_list.next); (typeof(*dev) *)( (char *)__mptr - ((size_t) &((typeof(*dev) *)0)->dev_list) );})) {
  for (i = 0; i < 35; i++) {
   if (gifconf_list[i]) {
    int done;
    if (!pos)
     done = gifconf_list[i](dev, 0, 0);
    else
