...
static inline struct net_device *first_net_device(struct net *net)
{
 return list_empty(&net->dev_base_head) ? 0 :
  ({ typeof( ((struct net_device *)0)->dev_list ) *__mptr = (net->dev_base_head.next); (struct net_device *)( (char *)__mptr - ((size_t) &((struct net_device *)0)->dev_list) );});
}
...
{
 return ({ typeof( ((struct sock *)0)->__sk_common.skc_node ) *__mptr = (head->first); (struct sock *)( (char *)__mptr - ((size_t) &((struct sock *)0)->__sk_common.skc_node) );});
}
...
static inline struct sock *sk_next( struct sock *sk)
{
 return sk->__sk_common.skc_node.next ?
  ({ typeof( ((struct sock *)0)->__sk_common.skc_node ) *__mptr = (sk->__sk_common.skc_node.next); (struct sock *)( (char *)__mptr - ((size_t) &((struct sock *)0)->__sk_common.skc_node) );}) : 0;
}
...
{
 if (!(sk->sk_state2.sk_userlocks & 1)) {
...
{
        int x = (waitall ? len : ({ int __min1 = (sk->sk_rcvlowat); int __min2 = (len); __min1 < __min2 ? __min1: __min2; }));
 return x ? x : 1;
}
...
{
 return queue->rskq_accept_head == 0;
}
...
{
 return queue->listen_opt != 0 ? queue->listen_opt->qlen : 0;
}
...
static inline int inet_sk_ehashfn( struct sock *sk)
{
 ...
 __be32 laddr = inet->rcv_saddr;
 __u16 lport = inet->num;
 __be32 faddr = inet->daddr;
 __be16 fport = inet->dport;
...
{
 if (sk->__sk_common.skc_prot->twsk_prot->twsk_unique != 0)
...
static inline void twsk_destructor(struct sock *sk)
{
 ...
 ;
 ;
 if (sk->__sk_common.skc_prot->twsk_prot->twsk_destructor != 0)
...
{
 return sk->__sk_common.skc_family == 10 ? __inet6_rcv_saddr(sk) : 0;
}
...
extern __typeof__(struct icmpv6msg_mib) *icmpv6msg_statistics[2];
struct ip6_ra_chain
{
...
static inline int ipv6_addr_equal( struct in6_addr *a1,
      struct in6_addr *a2)
{
...
static inline int __ipv6_prefix_equal( __be32 *a1, __be32 *a2,
...
static inline int ipv6_prefix_equal( struct in6_addr *a1,
        struct in6_addr *a2,
        unsigned int prefixlen)
{
...
static inline int ipv6_addr_loopback( struct in6_addr *a)
{
 return ((a->in6_u.u6_addr32[0] | a->in6_u.u6_addr32[1] |
   a->in6_u.u6_addr32[2] | (a->in6_u.u6_addr32[3] ^ (( __be32)__fswab32((1))))) == 0);
}

static inline int ipv6_addr_v4mapped( struct in6_addr *a)
{
 return ((a->in6_u.u6_addr32[0] | a->in6_u.u6_addr32[1] |
...
static inline int ipv6_addr_orchid( struct in6_addr *a)
{
...
extern __typeof__(struct linux_xfrm_mib) *xfrm_statistics[2];
...
static inline struct audit_buffer *xfrm_audit_start( char *op)
{
 struct audit_buffer *audit_buf = 0;

 if (audit_enabled == 0)
  return 0;
 audit_buf = audit_log_start(get_current()->audit_context, ((( gfp_t)0x20u)),
        1415);
 if (audit_buf == 0)
  return 0;
...
{
 if (__builtin_expect(!!(policy != 0), 1))
  ;
}
...
static inline int xfrm4_policy_check_reverse(struct sock *sk, int dir,
          struct sk_buff *skb)
{
 return __xfrm_policy_check2(sk, dir, skb, 2, 1);
}

static inline int xfrm6_policy_check_reverse(struct sock *sk, int dir,
          struct sk_buff *skb)
{
 return __xfrm_policy_check2(sk, dir, skb, 10, 1);
}
...
{
 if (__builtin_expect(!!(sk->sk_policy[0] != 0), 0)) {
  xfrm_policy_delete(sk->sk_policy[0], XFRM_POLICY_MAX);
  sk->sk_policy[0] = 0;
 }
 if (__builtin_expect(!!(sk->sk_policy[1] != 0), 0)) {
...
struct xfrm_policy *xfrm_policy_bysel_ctx(u8 type, int dir,
       struct xfrm_selector *sel,
       struct xfrm_sec_ctx *ctx, int _delete,
       int *err);
struct xfrm_policy *xfrm_policy_byid(u8, int dir, u32 id, int _delete, int *err);
...
extern int skb_icv_walk( struct sk_buff *skb, struct hash_desc *tfm,
...
struct cipher_desc {
 struct crypto_tfm *tfm;
 void (*crfn)(struct crypto_tfm *tfm, u8 *dst, u8 *src);
 unsigned int (*prfn)( struct cipher_desc *desc, u8 *dst,
        u8 *src, unsigned int nbytes);
 void *info;
};
...
struct ablkcipher_alg {
 int (*setkey)(struct crypto_ablkcipher *tfm, u8 *key,
               unsigned int keylen);
...
struct ahash_alg {
 ...
 int (*setkey)(struct crypto_ahash *tfm, u8 *key,
   unsigned int keylen);
...
struct aead_alg {
 int (*setkey)(struct crypto_aead *tfm, u8 *key,
               unsigned int keylen);
...
struct blkcipher_alg {
 int (*setkey)(struct crypto_tfm *tfm, u8 *key,
               unsigned int keylen);
...
struct cipher_alg {
 ...
 int (*cia_setkey)(struct crypto_tfm *tfm, u8 *key,
                   unsigned int keylen);
 void (*cia_encrypt)(struct crypto_tfm *tfm, u8 *dst, u8 *src);
 void (*cia_decrypt)(struct crypto_tfm *tfm, u8 *dst, u8 *src);
};

struct digest_alg {
 unsigned int dia_digestsize;
 void (*dia_init)(struct crypto_tfm *tfm);
 void (*dia_update)(struct crypto_tfm *tfm, u8 *data,
      unsigned int len);
 void (*dia_final)(struct crypto_tfm *tfm, u8 *out);
 int (*dia_setkey)(struct crypto_tfm *tfm, u8 *key,
                   unsigned int keylen);
};
...
struct hash_alg {
 ...
 int (*setkey)(struct crypto_hash *tfm, u8 *key,
        unsigned int keylen);
...
struct compress_alg {
 int (*coa_compress)(struct crypto_tfm *tfm, u8 *src,
       unsigned int slen, u8 *dst, unsigned int *dlen);
 int (*coa_decompress)(struct crypto_tfm *tfm, u8 *src,
         unsigned int slen, u8 *dst, unsigned int *dlen);
};
struct crypto_alg {
...
int crypto_has_alg( char *name, u32 type, u32 mask);
...
struct cipher_tfm {
 ...
 void (*cit_encrypt_one)(struct crypto_tfm *tfm, u8 *dst, u8 *src);
 void (*cit_decrypt_one)(struct crypto_tfm *tfm, u8 *dst, u8 *src);
};
...
struct crypto_tfm *crypto_alloc_base( char *alg_name, u32 type, u32 mask);
...
static inline char *crypto_tfm_alg_driver_name(struct crypto_tfm *tfm)
{
...
static inline char *crypto_tfm_alg_modname(struct crypto_tfm *tfm)
{
...
struct crypto_ablkcipher *crypto_alloc_ablkcipher( char *alg_name,
        u32 type, u32 mask);
...
static inline int crypto_has_ablkcipher( char *alg_name, u32 type,
     u32 mask)
{
...
{
 return ({ typeof( ((struct ablkcipher_request *)0)->base ) *__mptr = (req); (struct ablkcipher_request *)( (char *)__mptr - ((size_t) &((struct ablkcipher_request *)0)->base) );});
}
...
struct crypto_aead *crypto_alloc_aead( char *alg_name, u32 type, u32 mask);
...
static inline int crypto_aead_setkey(struct crypto_aead *tfm, u8 *key,
         unsigned int keylen)
{
...
static inline int crypto_has_blkcipher( char *alg_name, u32 type, u32 mask)
{
 type &= ~0x0000000f;
 type |= 0x00000004;
 mask |= 0x0000000f;
...
static inline char *crypto_blkcipher_name(struct crypto_blkcipher *tfm)
{
...
static inline struct crypto_cipher *crypto_alloc_cipher( char *alg_name,
       u32 type, u32 mask)
{
 type &= ~0x0000000f;
 type |= 0x00000001;
...
static inline int crypto_has_cipher( char *alg_name, u32 type, u32 mask)
{
 type &= ~0x0000000f;
 type |= 0x00000001;
 mask |= 0x0000000f;
...
static inline struct crypto_hash *crypto_alloc_hash( char *alg_name,
          u32 type, u32 mask)
{
 type &= ~0x0000000f;
 mask &= ~0x0000000f;
...
static inline int crypto_has_hash( char *alg_name, u32 type, u32 mask)
{
 type &= ~0x0000000f;
 mask &= ~0x0000000f;
 type |= 0x00000009;
...
static inline struct crypto_comp *crypto_alloc_comp( char *alg_name,
          u32 type, u32 mask)
{
 type &= ~0x0000000f;
 type |= 0x00000002;
...
static inline int crypto_has_comp( char *alg_name, u32 type, u32 mask)
{
 type &= ~0x0000000f;
 type |= 0x00000002;
 mask |= 0x0000000f;
...
static inline char *crypto_comp_name(struct crypto_comp *tfm)
{
...
void sha_transform(__u32 *digest, char *data, __u32 *W);

__u32 half_md4_transform(__u32 buf[4], __u32 in[8]);
...
static inline struct inet_ehash_bucket *_inet_ehash_bucket(
 struct inet_hashinfo *hashinfo,
 unsigned int hash)
{
...
static inline int inet_ehash_locks_alloc(struct inet_hashinfo *hashinfo)
{
 ...
 unsigned int nr_pcpus = __cpus_weight(&(cpu_possible_map), 64);
 ...
 if (sizeof(rwlock_t) != 0) {
  ...
  if (size * sizeof(rwlock_t) > ((1UL) << 12))
...
static inline int inet_lhashfn(struct net *net, unsigned short num)
{
 return (num + net_hash_mix(net)) & (32 - 1);
}

static inline int inet_sk_listen_hashfn( struct sock *sk)
{
      ...
      int dif)
{
 ...
 return sk ? sk : __inet_lookup_listener(net, hashinfo, daddr, hnum, dif);
}
...
{
 return __fswab16(( __u16)(__be16)(*( __be16 *)ipv6h)) >> 4;
}
...
static inline int tcp_too_many_orphans(struct sock *sk, int num)
{
 return (num > sysctl_tcp_max_orphans) ||
  (sk->sk_wmem_queued > 2048 &&
   ((&tcp_memory_allocated)->counter) > sysctl_tcp_mem[2]);
}
...
static inline u32 tcp_receive_window( struct tcp_sock *tp)
{
...
static inline void tcp_dec_pcount_approx_int(__u32 *count, int decr)
{
 if (*count) {
  *count -= decr;
  if ((int)*count < 0)
...
extern int tcp_set_default_congestion_control( char *name);
extern void tcp_get_default_congestion_control(char *name);
extern void tcp_get_available_congestion_control(char *buf, size_t len);
extern void tcp_get_allowed_congestion_control(char *buf, size_t len);
extern int tcp_set_allowed_congestion_control(char *allowed);
extern int tcp_set_congestion_control(struct sock *sk, char *name);
...
static inline void tcp_set_ca_state(struct sock *sk, u8 ca_state)
{
 struct inet_connection_sock *icsk = inet_csk(sk);
...
static inline void tcp_ca_event(struct sock *sk, enum tcp_ca_event event)
{
 struct inet_connection_sock *icsk = inet_csk(sk);
...
static inline int tcp_is_sack( struct tcp_sock *tp)
{
 return tp->rx_opt.sack_ok;
}

static inline int tcp_is_reno( struct tcp_sock *tp)
{
 return !tcp_is_sack(tp);
}

static inline int tcp_is_fack( struct tcp_sock *tp)
{
...
static inline unsigned int tcp_left_out( struct tcp_sock *tp)
{
...
static inline __u32 tcp_current_ssthresh( struct sock *sk)
{
 struct tcp_sock *tp = tcp_sk(sk);
...
extern void tcp_enter_cwr(struct sock *sk, int set_ssthresh);
...
static __inline__ __u32 tcp_max_burst( struct tcp_sock *tp)
{
 return tp->reordering;
}


static inline u32 tcp_wnd_end( struct tcp_sock *tp)
{
 return tp->snd_una + tp->snd_wnd;
}
extern int tcp_is_cwnd_limited( struct sock *sk, u32 in_flight);
...
static inline int tcp_prequeue(struct sock *sk, struct sk_buff *skb)
{
 ...
 if (!sysctl_tcp_low_latency && tp->ucopy.task) {
  ...
  if (tp->ucopy.memory > sk->sk_rcvbuf) {
   ...
   while ((skb1 = __skb_dequeue(&tp->ucopy.prequeue)) != 0) {
    sk->sk_backlog_rcv(sk, skb1);
    (({ struct percpu_data *__p = (struct percpu_data *)~(unsigned long)(((sock_net(sk))->mib.net_statistics[0])); (__typeof__(((sock_net(sk))->mib.net_statistics[0])))__p->ptrs[((({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; })))]; })->mibs[LINUX_MIB_TCPPREQUEUEDROPPED]++);
   }

   tp->ucopy.memory = 0;
  } else if (skb_queue_len(&tp->ucopy.prequeue) == 1) {
   __wake_up(sk->sk_sleep, 1, 1, 0);
...
static inline int keepalive_intvl_when( struct tcp_sock *tp)
{
 return tp->keepalive_intvl ? tp->keepalive_intvl : sysctl_tcp_keepalive_intvl;
}

static inline int keepalive_time_when( struct tcp_sock *tp)
{
 return tp->keepalive_time ? tp->keepalive_time : sysctl_tcp_keepalive_time;
}

static inline int tcp_fin_time( struct sock *sk)
{
 int fin_timeout = tcp_sk(sk)->linger2 ? tcp_sk(sk)->linger2 : sysctl_tcp_fin_timeout;
 int rto = inet_csk(sk)->icsk_rto;
...
static inline void tcp_mib_init(struct net *net)
{
 ...
 do { ({ struct percpu_data *__p = (struct percpu_data *)~(unsigned long)(((net)->mib.tcp_statistics[1])); (__typeof__(((net)->mib.tcp_statistics[1])))__p->ptrs[((({ do { } while (0); ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; }); })))]; })->mibs[TCP_MIB_RTOALGORITHM] += 1; do { } while (0); } while (0);
 do { ({ struct percpu_data *__p = (struct percpu_data *)~(unsigned long)(((net)->mib.tcp_statistics[1])); (__typeof__(((net)->mib.tcp_statistics[1])))__p->ptrs[((({ do { } while (0); ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; }); })))]; })->mibs[TCP_MIB_RTOMIN] += ((unsigned)(1000/5))*1000/1000; do { } while (0); } while (0);
 do { ({ struct percpu_data *__p = (struct percpu_data *)~(unsigned long)(((net)->mib.tcp_statistics[1])); (__typeof__(((net)->mib.tcp_statistics[1])))__p->ptrs[((({ do { } while (0); ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; }); })))]; })->mibs[TCP_MIB_RTOMAX] += ((unsigned)(120*1000))*1000/1000; do { } while (0); } while (0);
 do { ({ struct percpu_data *__p = (struct percpu_data *)~(unsigned long)(((net)->mib.tcp_statistics[1])); (__typeof__(((net)->mib.tcp_statistics[1])))__p->ptrs[((({ do { } while (0); ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; }); })))]; })->mibs[TCP_MIB_MAXCONN] += -1; do { } while (0); } while (0);
}
...
{
 tp->lost_skb_hint = 0;
 tp->scoreboard_skb_hint = 0;
 tp->retransmit_skb_hint = 0;
 tp->forward_skb_hint = 0;
}
...
{
 int cpu = ({ do { } while (0); ({ typeof(_proxy_pda.cpunumber) ret__; switch (sizeof(_proxy_pda.cpunumber)) { case 2: asm("mov" "w %%gs:%c1,%0" : "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 4: asm("mov" "l %%gs:%c1,%0": "=r" (ret__): "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; case 8: asm("mov" "q %%gs:%c1,%0": "=r" (ret__) : "i" (((size_t) &((struct x8664_pda *)0)->cpunumber)), "m" (_proxy_pda.cpunumber)); break; default: __bad_pda_field(); } ret__; }); });
...
static inline void tcp_write_queue_purge(struct sock *sk)
{
 ...
 while ((skb = __skb_dequeue(&sk->sk_write_queue)) != 0)
...
static inline void tcp_add_write_queue_tail(struct sock *sk, struct sk_buff *skb)
{
 ...
 if (sk->sk_send_head == 0) {
  sk->sk_send_head = skb;

  if (tcp_sk(sk)->highest_sack == 0)
...
static inline void tcp_insert_write_queue_before(struct sk_buff *_new,
        struct sk_buff *skb,
        struct sock *sk)
{
 __skb_insert(_new, skb->prev, skb, &sk->sk_write_queue);

 if (sk->sk_send_head == skb)
  sk->sk_send_head = _new;
}
...
static inline int tcp_skb_is_last( struct sock *sk,
      struct sk_buff *skb)
{
...
{
 tcp_sk(sk)->highest_sack = tcp_skb_is_last(sk, skb) ? 0 :
