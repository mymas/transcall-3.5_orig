...
extern dev_t name_to_dev_t(char *name);
           ...
           extern struct hlist_head unix_socket_table[256 + 1];
...
static struct sock *unix_seq_idx(struct seq_file *seq, loff_t pos)
{
 ...
 return 0;
           ...
           void *unix_seq_start(struct seq_file *seq, loff_t *pos)

{
 ...
 return *pos ? unix_seq_idx(seq, *pos - 1) : ((void *)1);
           ...
           void *unix_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
 struct unix_iter_state *iter = seq->_private;
 struct sock *sk = v;
 ++*pos;
...
static int unix_seq_show(struct seq_file *seq, void *v)
{
 ...
 if (v == ((void *)1))
  ...
  struct sock *s = v;
  struct unix_sock *u = ((struct unix_sock *)s);
  ;
  ...
  seq_printf(seq, "%p: %08X %08X %08X %04X %02X %5lu",
   ...
   0,
   s->__sk_common.skc_state == TCP_LISTEN ? (1 << 16) : 0,
   s->sk_type,
   s->sk_socket ?
   (s->__sk_common.skc_state == TCP_ESTABLISHED ? SS_CONNECTED : SS_UNCONNECTED) :
   (s->__sk_common.skc_state == TCP_ESTABLISHED ? SS_CONNECTING : SS_DISCONNECTING),
   sock_i_ino(s));
  ...
  if (u->addr) {
   ...
   if (!(((struct unix_sock *)s)->addr->hash != 256))
    len--;
   else {
    seq_putc(seq, '@');
    i++;
