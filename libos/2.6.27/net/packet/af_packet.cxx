...
static inline struct sock *packet_seq_idx(struct net *net, loff_t off)
{
 ...
 for (node = (&net->packet.sklist)->first; node && ({ __builtin_prefetch(node->next); 1;}) && ({ s = ({ typeof( ((typeof(*s) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*s) *)( (char *)__mptr - ((size_t) &((typeof(*s) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  if (!off--)
   return s;
 }
 return 0;
