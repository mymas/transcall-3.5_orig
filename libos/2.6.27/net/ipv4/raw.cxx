...
static inline struct icmphdr *icmp_hdr( struct sk_buff *skb)
{
...
static inline struct raw_sock *raw_sk( struct sock *sk)
{
...
extern struct hlist_head udp_hash[128];
...
extern __typeof__(struct udp_mib) *udplite_stats_in6[2];
...
int raw_seq_open(struct inode *ino, struct file *file,
   struct raw_hashinfo *h, struct seq_operations *ops);
...
extern struct list_head nf_hooks[35][8];
...
extern struct nf_afinfo *nf_afinfo[35];
...
nf_nat_decode_session(struct sk_buff *skb, struct flowi *fl, int family)
{
 ...
 if (family == 2) {
  ;
  decodefn = (ip_nat_decode_session);
  if (decodefn)
   decodefn(skb, fl);
  ;
 }
...
static inline struct net *nf_pre_routing_net( struct net_device *in,
          struct net_device *out)
{
...
static inline struct net *nf_local_in_net( struct net_device *in,
       struct net_device *out)
{
...
static inline struct net *nf_forward_net( struct net_device *in,
      struct net_device *out)
{
...
static inline struct net *nf_local_out_net( struct net_device *in,
        struct net_device *out)
{
...
static inline struct net *nf_post_routing_net( struct net_device *in,
           struct net_device *out)
{
...
{
 struct sock *sk;
 struct raw_iter_state* state = ((struct raw_iter_state *)(seq)->_private);

 for (state->bucket = 0; state->bucket < 256;
   ++state->bucket) {
  struct hlist_node *node;

  for (node = (&state->h->ht[state->bucket])->first; node && ({ __builtin_prefetch(node->next); 1;}) && ({ sk = ({ typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next)
...
static struct sock *raw_get_idx(struct seq_file *seq, loff_t pos)
{
 ...
 if (sk)
  while (pos && (sk = raw_get_next(seq, sk)) != 0)
   --pos;
 return pos ? 0 : sk;
}
...
extern typeof(raw_seq_start) raw_seq_start; static char __kstrtab_raw_seq_start[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "raw_seq_start"; static struct kernel_symbol __ksymtab_raw_seq_start __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&raw_seq_start, __kstrtab_raw_seq_start };
...
void *raw_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
 ...
 if (v == ((void *)1))
  sk = raw_get_first(seq);
                ...
                sk = raw_get_next(seq, (struct sock *)v);
 ++*pos;
 return sk;
}
extern typeof(raw_seq_next) raw_seq_next; static char __kstrtab_raw_seq_next[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "raw_seq_next"; static struct kernel_symbol __ksymtab_raw_seq_next __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&raw_seq_next, __kstrtab_raw_seq_next };
...
extern typeof(raw_seq_stop) raw_seq_stop; static char __kstrtab_raw_seq_stop[] __attribute__((section("__ksymtab_strings"), aligned(1))) = "" "raw_seq_stop"; static struct kernel_symbol __ksymtab_raw_seq_stop __attribute__((__used__)) __attribute__((section("__ksymtab" "_gpl"), unused)) = { (unsigned long)&raw_seq_stop, __kstrtab_raw_seq_stop };
...
static void raw_sock_seq_show(struct seq_file *seq, struct sock *sp, int i)
{
 ...
 seq_printf(seq, "%4d: %08X:%04X %08X:%04X"
  ...
  ((&sp->__sk_common.skc_refcnt)->counter), sp, ((&sp->sk_drops)->counter));
}
 ...
 if (v == ((void *)1))
  seq_printf(seq, "  sl  local_address rem_address   st tx_queue "
    ...
    "inode ref pointer drops\n");
                ...
                raw_sock_seq_show(seq, (struct sock *)v, ((struct raw_iter_state *)(seq)->_private)->bucket);
 return 0;
}
struct seq_operations raw_seq_ops = {
    raw_seq_start,
    raw_seq_stop,
    raw_seq_next,
    raw_seq_show,
};
void raw_set_private(struct raw_iter_state *state)
{
    state->h = &raw_v4_hashinfo;
}
