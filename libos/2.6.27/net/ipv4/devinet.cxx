...
extern __be32 inet_select_addr( struct net_device *dev, __be32 dst, int scope);
...
in_dev_get( struct net_device *dev)
{
...
static __inline__ struct in_device *
__in_dev_get_rtnl( struct net_device *dev)
{
...
extern void arp_send(int type, int ptype, __be32 dest_ip,
    ...
    unsigned char *src_hw, unsigned char *th);
...
{
 struct fib_rule *rule = ({ typeof( ((struct fib_rule *)0)->rcu ) *__mptr = (head); (struct fib_rule *)( (char *)__mptr - ((size_t) &((struct fib_rule *)0)->rcu) );});
...
extern int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) fib4_rules_init(struct net *net);
extern void __attribute__ ((__section__(".ref.text"))) __attribute__((noinline)) fib4_rules_exit(struct net *net);
...
extern void fib_select_default(struct net *net, struct flowi *flp,
...
extern void fib_select_multipath( struct flowi *flp, struct fib_result *res);
 ...
 *itag = ((*res).fi->fib_nh[(*res).nh_sel]).nh_tclassid<<16;
...
extern int __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) fib_proc_init(struct net *net);
extern void __attribute__ ((__section__(".ref.text"))) __attribute__((noinline)) fib_proc_exit(struct net *net);
           int inet_gifconf(struct net_device *dev, char *buf, int len)
{
 ...
 if (!in_dev || (ifa = in_dev->ifa_list) == 0)
  goto out;
 ...
 for (; ifa; ifa = ifa->ifa_next) {
  ...
  if (ifa->ifa_label)
   strcpy(ifr.ifr_ifrn.ifrn_name, ifa->ifa_label);
  else
