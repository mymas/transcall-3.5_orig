...
static inline struct igmphdr *igmp_hdr( struct sk_buff *skb)
{
 return (struct igmphdr *)skb_transport_header(skb);
}

static inline struct igmpv3_report *
   igmpv3_report_hdr( struct sk_buff *skb)
{
 return (struct igmpv3_report *)skb_transport_header(skb);
}

static inline struct igmpv3_query *
   igmpv3_query_hdr( struct sk_buff *skb)
{
...
extern struct hlist_head udplite_hash[128];
...
static inline int udplite_checksum_init(struct sk_buff *skb, struct udphdr *uh)
{
 ...
 if (uh->check == 0) {
  do { if (net_msg_warn && net_ratelimit()) printk("<7>" "UDPLITE: zeroed checksum field\n"); } while(0);
  return 1;
 }
 ...
 if (cscov == 0)
  ...
  do { if (net_msg_warn && net_ratelimit()) printk("<7>" "UDPLITE: bad csum coverage %d/%d\n", cscov, skb->len); } while(0)
                          ;
  return 1;
...
extern int udplite_get_port(struct sock *sk, unsigned short snum,
   int (*scmp)( struct sock *, struct sock *));
struct net_protocol {
 int (*handler)(struct sk_buff *skb);
...
{
 struct sock *sk;
 struct udp_iter_state *state = seq->_private;
 struct net *net = seq_file_net(seq);

 for (state->bucket = 0; state->bucket < 128; ++state->bucket) {
  struct hlist_node *node;
  for (node = (state->hashtable + state->bucket)->first; node && ({ __builtin_prefetch(node->next); 1;}) && ({ sk = ({ typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
...
try_again:
  ;
 } while (sk && (!net_eq(sock_net(sk), net) || sk->__sk_common.skc_family != state->family));
...
static struct sock *udp_get_idx(struct seq_file *seq, loff_t pos)
{
 ...
 if (sk)
  while (pos && (sk = udp_get_next(seq, sk)) != 0)
   --pos;
 return pos ? 0 : sk;
}
           ...
           void *udp_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
 ...
 if (v == ((void *)1))
  ...
  sk = udp_get_next(seq, (struct sock *)v);
  ...
  int bucket, int *len)
{
 ...
 __u16 destp = __fswab16(( __u16)(__be16)(inet->dport));
 __u16 srcp = __fswab16(( __u16)(__be16)(inet->sport));
...
int udp4_seq_show(struct seq_file *seq, void *v)
{
 if (v == ((void *)1))
  ...
  udp4_format_sock((struct sock *)v, seq, state->bucket, &len);
  seq_printf(seq, "%*s\n", 127 - len ,"");
 }
 return 0;
}
...
struct seq_operations udp_seq_ops = {
    udp_seq_start,
    udp_seq_stop,
    udp_seq_next,
    udp4_seq_show,
};
void udp_set_private(struct udp_iter_state *state)
{
    state->hashtable = udp_hash;
    state->family = 2;
}
