...
static int tcp4_seq_show(struct seq_file *seq, void *v)
{
	...
	case TCP_SEQ_STATE_LISTENING:
	case TCP_SEQ_STATE_ESTABLISHED:
		get_tcp4_sock((struct sock *)v, seq, st->num, &len);
		break;
	case TCP_SEQ_STATE_OPENREQ:
		get_openreq4(st->syn_wait_sk, (struct request_sock *)v, seq, st->num, st->uid, &len);
		break;
	case TCP_SEQ_STATE_TIME_WAIT:
		get_timewait4_sock((struct inet_timewait_sock *)v, seq, st->num, &len);
		break;
	}
	seq_printf(seq, "%*s\n", TMPSZ - 1 - len, "");
out:
...
#ifndef NOTC
struct seq_operations tcp_seq_ops = {
    tcp_seq_start,
    tcp_seq_stop,
    tcp_seq_next,
    tcp4_seq_show,
...
#ifndef NOTC
void tcp_set_private(struct tcp_iter_state *state)
{
