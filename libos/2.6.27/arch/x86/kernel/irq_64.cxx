...
{
 u64 sum = (_cpu_pda[cpu])->__nmi_count;

 sum += (_cpu_pda[cpu])->apic_timer_irqs;

 sum += (_cpu_pda[cpu])->irq_resched_count;
 sum += (_cpu_pda[cpu])->irq_call_count;
 sum += (_cpu_pda[cpu])->irq_tlb_count;


 sum += (_cpu_pda[cpu])->irq_thermal_count;
 sum += (_cpu_pda[cpu])->irq_threshold_count;

 sum += (_cpu_pda[cpu])->irq_spurious_count;
 return sum;
}

u64 arch_irq_stat(void)
{
 return ((&irq_err_count)->counter);
}
