 ...
 struct pci_dev *pci_dev, char cap)
{
 ...
 for (pos = (&pci_dev->saved_cap_space)->first; pos && ({ __builtin_prefetch(pos->next); 1;}) && ({ tmp = ({ typeof( ((typeof(*tmp) *)0)->next ) *__mptr = (pos); (typeof(*tmp) *)( (char *)__mptr - ((size_t) &((typeof(*tmp) *)0)->next) );}); 1;}); pos = pos->next) {
  if (tmp->cap_nr == cap)
   return tmp;
 }
 return 0;
...
static inline struct pci_bus * __attribute__ ((__section__(".devinit.text"))) __attribute__((__cold__)) pci_scan_bus(int bus, struct pci_ops *ops,
        void *sysdata)
{
 struct pci_bus *root_bus;
 root_bus = pci_scan_bus_parented(0, bus, ops, sysdata);
 if (root_bus)
...
int __attribute__((warn_unused_result)) pci_request_regions(struct pci_dev *, char *);
void pci_release_regions(struct pci_dev *);
int __attribute__((warn_unused_result)) pci_request_region(struct pci_dev *, int, char *);
...
pci_alloc_consistent(struct pci_dev *hwdev, size_t size,
       dma_addr_t *dma_handle)
{
 return dma_alloc_coherent(hwdev == 0 ? 0 : &hwdev->dev, size, dma_handle, ((( gfp_t)0x20u)));
}
...
pci_free_consistent(struct pci_dev *hwdev, size_t size,
      void *vaddr, dma_addr_t dma_handle)
{
 dma_free_coherent(hwdev == 0 ? 0 : &hwdev->dev, size, vaddr, dma_handle);
}

static inline dma_addr_t
pci_map_single(struct pci_dev *hwdev, void *ptr, size_t size, int direction)
{
 return dma_map_single(hwdev == 0 ? 0 : &hwdev->dev, ptr, size, (enum dma_data_direction)direction);
}
...
pci_unmap_single(struct pci_dev *hwdev, dma_addr_t dma_addr,
   size_t size, int direction)
{
 dma_unmap_single(hwdev == 0 ? 0 : &hwdev->dev, dma_addr, size, (enum dma_data_direction)direction);
}
...
pci_map_sg(struct pci_dev *hwdev, struct scatterlist *sg,
    int nents, int direction)
{
 return dma_map_sg(hwdev == 0 ? 0 : &hwdev->dev, sg, nents, (enum dma_data_direction)direction);
}
...
pci_unmap_sg(struct pci_dev *hwdev, struct scatterlist *sg,
      int nents, int direction)
{
 dma_unmap_sg(hwdev == 0 ? 0 : &hwdev->dev, sg, nents, (enum dma_data_direction)direction);
}
...
int pcim_iomap_regions(struct pci_dev *pdev, u16 mask, char *name);
...
extern void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) pci_mmcfg_early_init(void);
extern void __attribute__ ((__section__(".init.text"))) __attribute__((__cold__)) pci_mmcfg_late_init(void);
...
extern int e820_find_active_region( struct e820entry *ei,
      unsigned long start_pfn,
...
static struct vm_area_struct gate_vma = {
    ...
    (-10UL << 20) + (1 * ((1UL) << 12)),
    0,
    ((pgprot_t) { ((((pteval_t)(1)) << 0) | (((pteval_t)(1)) << 2) | (((pteval_t)(1)) << 5)) } ),
...
struct vm_area_struct *get_gate_vma(struct task_struct *tsk)
{
 ...
 if (test_tsk_thread_flag(tsk, 17))
  return 0;
...
int in_gate_area_no_task(unsigned long addr)
{
 return (addr >= (-10UL << 20)) && (addr < (-2UL << 20));
}
